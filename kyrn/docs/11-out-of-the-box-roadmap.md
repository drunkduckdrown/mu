...
更新日期：2026-09-22（上午更新）。来源：用户当天给出的目标（内置浏览器与 UltraFast 深度融合、Windows/WSL、设置与模型提供商、粉紫主题与界面打磨、本地 Laya 一键挂载、开箱即用的底线能力、按任务披露的能力包）。本文把目标拆成工作包，写明每一包的做法、先后和现状。状态随提交更新。
...
| checkpoint + 判断回退 | 每个用户回合用影子 git 目录给工作区拍快照（不碰用户仓库的 index）。回退 = 文件回到快照 + 会话树回到对应条目。`turn.rewind`：monitor 发现反复失败时由 Jev 判断是不是死路，再向用户提议 | **已完成**（功能 `checkpoint`，判定点 `turn.rewind`，设计见 `features/checkpoints.md`）：每个项目一个影子仓库（`~/.mu/agent/mu/checkpoints/<项目哈希>`，自己的 `GIT_DIR` / `GIT_WORK_TREE` / `GIT_INDEX_FILE`），用户的 index、stash、钩子从不触碰，非 git 目录也能用；快照只拍在回合第一次会改文件的调用之前，记成 `kyrn.checkpoint` 条目跟着会话树走。`/checkpoints` 列出，`/rewind [#] [both\|files\|conversation]` 先展示会放回/删除/找回哪些文件、单独问手改过的文件，对话用 pi 自己的 `navigateTree` 移回请求之前并留一句 `kyrn.rewind` 教训，`/rewind undo` 撤销。同一命令失败 3 次或 monitor 第二次开口时问 Jev（`dead_end` / `progress` 两个是非题），确信是死路才**提议**，用户同意才回退，没界面时只提醒模型。未验证：Windows、真实 Jev 的阈值、真实终端里的提议流程。待你定：要不要加模型自己请求回退的 `rewind` 工具（本包未做） |
...
**目标模式：已完成，第二版**（`/goal <条件>`，只输入 `/goal` 会问条件；设计见 `features/goal-mode.md`）。用户反馈“Jev 判断结束不准”，所以第二版改成**由大模型判断**（默认当前会话的模型，可另指定）：它读目标、这一轮的步骤、最近一次测试 / 检查的结果和结束语，给出达成 / 继续 / 需要你，外加理由和具体的下一步，续跑消息带上这两样；判断为原地打转时要它换思路，连续两次就停下来问用户。事实（没勾的验收条件、改完没跑）照样压过任何结论；Jev 的 `goal.met` 退为兜底。模型需要用户、被打断、调用失败、连续空转、额度用完时自己停下，用户的下一条消息就是“接着干”。

**人话看板：harness 已完成**（用户 2026-09-22 追加，`/board on|off` 按项目开关，设计见 `features/plain-language-board.md`）。Jev 用选择题读出阶段、在做哪条验收条件、是否在等用户、有没有新情况；只有出现新情况，才请会说人话的模型（`board.model`，比如 Gemini Flash）写“进展 / 正在做 / 需要你确认”。只给人看，不进模型的上下文；展示事件 `board.update`。桌面端右侧面板（和经验栏放在一起）待做。

**用 Google 账号登录：harness 已完成，实验**（用户 2026-09-22 同意，功能 `googleLogin`，设计见 `features/google-login.md`）。上游 4 月删掉的 Gemini CLI 和 Antigravity 的 OAuth 加回来了，登录前先说明封号风险、用户同意才继续；Gemini CLI 用 Google 自己的 Gemini 目录（含 3.8 Flash），Antigravity 登录后按账号实际能用的模型更新列表。官方方式（Gemini API key、Vertex 的 gcloud 登录）和 Grok 的登录（`xai`）都是 pi 自带的。没有用真实账号登录过；桌面端的登录界面待做（pi 的 RPC 没有登录命令，要在进程内调 `ModelRuntime.login`）。
...
| 本地 Laya 一键挂载 | 设置里一个按钮完成安装（运行时 + 权重，带进度），装好后把判定层级切到 `laya` 并如实标注它的能力边界（分类可用，关联/打分不可信）。没有 Jev 的用户因此也能用 | 待做。用户 2026-09-22 批准做，条件是**一定要用户同意才下载**：模型（约 930 MB）只在用户在界面里明确点了下载之后才开始下，下载前说清大小和来源 |
...
| 2. 新代码跨平台 | 进程用 `spawn` 加参数数组、不经 shell；tsx 走真正的 JS 入口而不是 `.cmd` 垫片；子代理在 Windows 上用 `taskkill /T` 整树结束（MCP 客户端、后台命令已各自处理） | 已完成，未在真机验证。合并时补上：风险把关、硬约束把关、完成核对、checkpoint、蜂群视图都认 pi 在 Windows 上的 `powershell` 工具（`src/extension/shell-tools.ts`），风险规则加了 PowerShell / cmd 的同类写法（`Remove-Item -Recurse -Force`、`rd /s /q`、`iex (iwr …)`、`-Verb RunAs`、`Format-Volume`），只读命令表加了 `Get-ChildItem` / `Get-Content` / `Select-String` 等 |
