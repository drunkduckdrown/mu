...
- **不经过 `.cmd` 垫片。** Windows 上 `node_modules/.bin/tsx` 是 `.cmd`，只能经 shell 启动，带引号或 `&` 的 prompt 会被二次解析。所有平台都改成：读 `node_modules/tsx/package.json` 的 `bin`，拿到真正的 `dist/cli.mjs`，用 `process.execPath` 加参数数组启动，从不拼命令行字符串。
