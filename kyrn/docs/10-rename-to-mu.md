...
迁移由 `mu migrate` 完成（**尚未在真实机器上执行**，需要用户明确同意）。它只做三件事，全部是目录级操作，不打开、不复制、不改写里面任何文件，`auth.json`、`models-store.json` 的内容和 600 权限原样跟着走：
...
没验证：在用户真实数据上的原地改名（上面的检查用的是空的临时数据目录）、旧对话能否打开、JeV 面板和蜂群事件。

## 6. 还没做的

1. **数据迁移**（第 3 节）：命令已就绪，等用户同意后执行 `mu judge stop && mu migrate`。
