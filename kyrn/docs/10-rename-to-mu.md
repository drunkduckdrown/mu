...
更新日期：2026-09-22。状态：**第一阶段的代码（harness 和桌面端）已完成并验证；数据迁移和实机冒烟尚未开始。**
...
- **注册按 command 找，不按显示名找。** 原来 `product.ts` 和 `scripts/kyrn/register.mjs` 都用 `name === 'KYRN'` 找后端里的代理：直接改显示名会另注册出一个 “mu”，旧对话还挂在旧代理上，“只启用自己”的逻辑还会把旧代理停掉。现在：command 相同的那一行就是自己的注册，不管它叫什么；名字不是 `mu` 就原地改名。AionCore v0.2.2 的 `PUT /api/agents/custom/{id}` 是**整体替换**（漏传的图标、描述、参数、环境变量会被清空），所以把读到的整行原样带回去，只换名字；描述只有还是我们自己写的旧文案时才换，用户改过的保留。后端保存前会探活，探活失败返回 400：改名失败**不挡启动**，注册照旧名工作，下次启动再试。同名（新旧名都算）但 command 不同的行仍然拒绝，绝不重复注册。两处代码规则一致。
...
验证：`tests/unit/kyrn`、侧栏品牌、窗口标题共 13 个测试文件 116 个用例通过，其中新增 9 个（原地改名且整行带回、保留用户自己写的描述、改名被拒不挡启动、已是新名不再 PUT、未注册时只注册一次、同名异 command 拒绝；`kyrn.json`/`mu.json` 的读写跟随；凭据变量两种前缀；数据目录规则）。`tsc --noEmit` 0 错，改动文件的 Oxlint 0 警告、Oxfmt 通过，i18n 类型已是最新、13 种语言校验通过。冒烟：用临时 HOME 直接启动 ACP 适配器并发 `initialize`，返回 `title: "mu"`，真实的 `~/.mu` 没有被创建。

没验证：真实桌面端启动后的原地改名（需要 AionCore 在跑）、旧对话能否打开、JeV 面板和蜂群事件。

## 6. 还没做的

1. **数据迁移**（第 3 节）。
2. **实机冒烟**：第 4 节和第 5 节的“没验证”。
