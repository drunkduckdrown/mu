...
!define AIONUI_MSG_INSTALL_FAILED_ZH "mu 安装失败"
!define AIONUI_MSG_INSTALL_FAILED_EN "mu installation failed"
...
!define AIONUI_MSG_VERIFY_REQUIRED_FILE_ZH "mu 安装不完整，缺少必需文件："
!define AIONUI_MSG_VERIFY_REQUIRED_FILE_ACTION_EN "Please reinstall mu or download a newer installer."
!define AIONUI_MSG_VERIFY_REQUIRED_FILE_ACTION_ZH "请重新安装 mu，或下载更新版本的安装器。"
...
!define AIONUI_MSG_ARCH_MISMATCH_ACTION_EN "Download the mu installer that matches this Windows architecture, then run it again."
!define AIONUI_MSG_ARCH_MISMATCH_ACTION_ZH "请下载与当前 Windows 架构匹配的 mu 安装器，然后再次运行。"

!define AIONUI_MSG_UNINSTALLER_COPY_LOCKED_EN "mu could not overwrite the installed uninstaller because it is locked."
!define AIONUI_MSG_UNINSTALLER_COPY_LOCKED_ZH "mu 无法覆盖已安装的卸载器，因为该文件被占用。"
!define AIONUI_MSG_UNINSTALLER_REBUILD_FAILED_EN "mu could not rebuild the missing installed uninstaller."
!define AIONUI_MSG_UNINSTALLER_REBUILD_FAILED_ZH "mu 无法重建缺失的已安装卸载器。"
!define AIONUI_MSG_UNINSTALLER_REBUILD_MISSING_EN "mu rebuilt the uninstaller, but the rebuilt file is still missing."
!define AIONUI_MSG_UNINSTALLER_REBUILD_MISSING_ZH "mu 已尝试重建卸载器，但重建后的文件仍然缺失。"
!define AIONUI_MSG_UNINSTALLER_REPAIR_ACTION_EN "Close mu, restart Windows if needed, then run this installer again."
!define AIONUI_MSG_UNINSTALLER_REPAIR_ACTION_ZH "请关闭 mu，必要时重启 Windows，然后再次运行此安装器。"

!define AIONUI_MSG_OLD_UNINSTALL_FAILED_EN "The previous mu uninstaller returned an error."
!define AIONUI_MSG_OLD_UNINSTALL_FAILED_ZH "之前的 mu 卸载器返回错误。"
...
!define AIONUI_MSG_REPLACE_LOCKED_EN "mu could not safely replace the previous installation because files are still open."
!define AIONUI_MSG_REPLACE_LOCKED_ZH "mu 无法安全替换之前的安装，因为仍有文件处于打开状态。"
!define AIONUI_MSG_PREVIOUS_FILE_OPEN_EN "mu cannot continue because a file in the previous installation is still open."
!define AIONUI_MSG_PREVIOUS_FILE_OPEN_ZH "mu 无法继续，因为之前安装目录中的某个文件仍处于打开状态。"
!define AIONUI_MSG_REMOVE_PREVIOUS_DIR_EN "mu could not remove the previous installation directory."
!define AIONUI_MSG_REMOVE_PREVIOUS_DIR_ZH "mu 无法删除之前的安装目录。"
...
!define AIONUI_MSG_CLOSE_INSTALL_DIR_ACTION_EN "Close mu and any file browsers in the install directory, then run this installer again."
!define AIONUI_MSG_CLOSE_INSTALL_DIR_ACTION_ZH "请关闭 mu 以及任何打开安装目录的文件管理器，然后再次运行安装器。"

!define AIONUI_MSG_APP_CANNOT_BE_CLOSED_EN "mu could not finish closing or removing the previous version.$\r$\n$\r$\nClose mu and any program that may be using files in the install folder, then click Retry.$\r$\n$\r$\nIf Retry keeps returning here, click Cancel. The installer will show the failed path, any program reported by Windows Restart Manager, and the installer log."
!define AIONUI_MSG_APP_CANNOT_BE_CLOSED_ZH "mu 无法完成关闭或移除旧版本。$\r$\n$\r$\n请关闭 mu 以及任何可能正在使用安装目录中文件的程序，然后点击重试。$\r$\n$\r$\n如果重试后仍然回到这里，请点击取消。安装器会显示失败路径、Windows Restart Manager 报告的占用程序以及安装日志。"
...
!define AIONUI_MSG_FILE_OR_FOLDER_IN_USE_EN "mu cannot continue because a file or folder in the install directory is still in use:"
...
!define AIONUI_MSG_RETRY_AFTER_CLOSING_DIR_EN "Click Retry after closing mu and any program using that folder. Click Cancel to show the diagnostics and installer log."
!define AIONUI_MSG_RETRY_AFTER_CLOSING_DIR_ZH "请关闭 mu 以及任何使用该文件夹的程序后点击重试。点击取消会显示诊断信息和安装日志。"
