...
describe('unmappedStartError (a start failure without an error code)', () => {
  it('leads with the translated headline and keeps the raw error only as a detail', async () => {
    const { unmappedStartError } =
      await import('@/renderer/pages/conversation/Preview/components/viewers/OfficeWatchViewer');
    expect(unmappedStartError(new Error('HTTP 500: Internal Server Error'), 'PPT 预览启动失败')).toEqual({
      message: 'PPT 预览启动失败',
      detail: 'HTTP 500: Internal Server Error',
    });
  });

  it('adds no detail when there is nothing more to say', async () => {
    const { unmappedStartError } =
      await import('@/renderer/pages/conversation/Preview/components/viewers/OfficeWatchViewer');
    expect(unmappedStartError('boom', 'Failed')).toEqual({ message: 'Failed', detail: undefined });
    expect(unmappedStartError(new Error('  '), 'Failed')).toEqual({ message: 'Failed', detail: undefined });
    expect(unmappedStartError(new Error('Failed'), 'Failed')).toEqual({ message: 'Failed', detail: undefined });
  });
});
