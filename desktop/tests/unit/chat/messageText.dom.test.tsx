...
describe('message time', () => {
  ...
  it('shows the time today and the date on other days, in the app language', () => {
    const today = new Date(2026, 8, 22, 9, 5).getTime();
    const earlier = new Date(2026, 8, 21, 9, 5).getTime();
    const lastYear = new Date(2025, 11, 31, 21, 5).getTime();
    expect(formatMessageTime(today, 'en-US', now)).toBe('09:05 AM');
    expect(formatMessageTime(today, 'zh-CN', now)).toBe('09:05');
    expect(formatMessageTime(earlier, 'en-US', now)).toBe('09/21, 09:05 AM');
    expect(formatMessageTime(earlier, 'zh-CN', now)).toBe('09/21 09:05');
    expect(formatMessageTime(lastYear, 'zh-CN', now)).toBe('2025/12/31 21:05');
  });
});
