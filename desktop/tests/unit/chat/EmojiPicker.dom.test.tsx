...
describe('EmojiPicker', () => {
  ...
  it('names the emoji categories through translation keys', () => {
    render(
      <ConfigProvider>
        <EmojiPicker>
          <button type='button'>Open picker</button>
    ...
    fireEvent.click(screen.getByText('Open picker'));
    expect(screen.getByTitle('settings.emojiCategories.smileys')).toBeInTheDocument();
    expect(screen.getByTitle('settings.emojiCategories.flags')).toBeInTheDocument();
    expect(screen.queryByTitle('Smileys')).toBeNull();
  });
});
