...
describe('MessageAcpToolCall image output', () => {
  ...
  it('renders kind fallback labels when title is missing', () => {
    const cases: Array<[IMessageAcpToolCall['content']['update']['kind'], string]> = [
      ['edit', 'tools.kind.edit'],
      ['read', 'tools.kind.read'],
      ['execute', 'tools.shell.displayName'],
  ...
  it('renders status labels for pending and in-progress states', () => {
    ...
    expect(screen.getByText('common.kyrn.toolStatus.pending')).toBeInTheDocument();
    ...
    expect(screen.getByText('common.kyrn.toolStatus.in_progress')).toBeInTheDocument();
  });

  it('localizes completed and failed, and shows an unknown status as the agent sent it', () => {
    const { rerender } = render(<MessageAcpToolCall message={createMessage({ ...baseUpdate, status: 'completed' })} />);
    expect(screen.getByText('common.kyrn.toolStatus.completed')).toBeInTheDocument();

    rerender(<MessageAcpToolCall message={createMessage({ ...baseUpdate, status: 'failed' })} />);
    expect(screen.getByText('common.kyrn.toolStatus.failed')).toBeInTheDocument();

    rerender(
      <MessageAcpToolCall
        message={createMessage({ ...baseUpdate, status: 'paused' as unknown as typeof baseUpdate.status })}
      />
    );
  ...
  it('shows a diff-only file change without duplicate tool details', () => {
    ...
    expect(screen.queryByText(/tools.labels.toolCallId/)).not.toBeInTheDocument();
