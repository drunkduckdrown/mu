...
describe('DefaultsSection thought level options', () => {
  it("names pi's levels in the app language and keeps an agent's own level names", () => {
    renderDefaultsSection({
      showThoughtLevelDefault: true,
      thoughtLevelOptions: [
    ...
    const select = screen.getByTestId('select-assistant-default-thought-level');
    expect(select.querySelector('[data-value="xhigh"]')).toHaveTextContent(/^mu\.levels\.xhigh$/);
    expect(select.querySelector('[data-value="turbo"]')).toHaveTextContent(/^Turbo$/);
  });
});
