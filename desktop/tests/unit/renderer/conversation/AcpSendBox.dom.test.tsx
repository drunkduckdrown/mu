...
const appCommand = () =>
  vi.mocked(useAddEventListener).mock.calls.findLast(([name]) => name === 'sendbox.command')?.[1] as unknown as (
    text: string,
    target: string,
    heard?: (state: string) => void
  ) => void;
...
describe('AcpSendBox', () => {
  ...
  it('sends a command the app sends for the person at once when idle, only in its own conversation', async () => {
    sendMessageInvokeMock.mockResolvedValue({ turn_id: 'turn-1', runtime: null, msg_id: 'msg-1' });
    render(
      <AcpSendBox
        conversation_id='conv-1'
    ...
    const heard = vi.fn();
    await act(async () => appCommand()('/board on', 'conv-2', heard));
    expect(sendMessageInvokeMock).not.toHaveBeenCalled();
    await act(async () => appCommand()('/board on', 'conv-1', heard));
    await waitFor(() => expect(heard).toHaveBeenCalledWith('sent'));
    expect(sendMessageInvokeMock).toHaveBeenCalledWith(
      expect.objectContaining({ input: '/board on', conversation_id: 'conv-1', files: [] })
    );
    // Never into the message queue, which holds items until they are sent by hand.
    expect(enqueueMock).not.toHaveBeenCalled();
  });

  it('while busy, keeps the latest command and sends it when the turn ends; leaving drops it', async () => {
    sendMessageInvokeMock.mockResolvedValue({ turn_id: 'turn-1', runtime: null, msg_id: 'msg-1' });
    runtimeViewMock.isProcessing = true;
    // A new element each time: the same one again would not render the box again.
    const box = () => (
      <AcpSendBox
        conversation_id='conv-1'
        backend='kyrn'
    ...
    const { rerender, unmount } = render(box());
    const first = vi.fn();
    const second = vi.fn();
    await act(async () => appCommand()('/board on', 'conv-1', first));
    await act(async () => appCommand()('/board off', 'conv-1', second));
    expect(first.mock.calls).toEqual([['waiting'], ['dropped']]);
    ...
    runtimeViewMock.isProcessing = false;
    rerender(box());
    await waitFor(() => expect(second).toHaveBeenCalledWith('sent'));
    expect(sendMessageInvokeMock).toHaveBeenCalledTimes(1);
    expect(sendMessageInvokeMock).toHaveBeenCalledWith(expect.objectContaining({ input: '/board off' }));

    runtimeViewMock.isProcessing = true;
    rerender(box());
    const third = vi.fn();
    await act(async () => appCommand()('/board on', 'conv-1', third));
    unmount();
    expect(third.mock.calls).toEqual([['waiting'], ['dropped']]);
    expect(sendMessageInvokeMock).toHaveBeenCalledTimes(1);
  });
