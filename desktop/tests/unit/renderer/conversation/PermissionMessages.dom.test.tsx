...
describe('permission message adapters', () => {
  ...
  it('keeps the generic payload exact and defaults confirmation to proceed_once', async () => {
    ...
    expect(screen.getByText('messages.permissionKind.execute')).toBeInTheDocument();
  ...
  it('keeps the ACP payload exact and defaults confirmation to allow_once', async () => {
    ...
    expect(screen.getByText('messages.permissionKind.edit')).toBeInTheDocument();
  ...
  it('preserves generic fallback IDs, labels, title, and empty msg_id', async () => {
    ...
    expect(screen.getByText('messages.permissionRequest')).toBeInTheDocument();
    expect(screen.getByText('messages.permissionKind.tool')).toBeInTheDocument();
  ...
  it('renders a safe generic fallback for malformed confirmation content', () => {
    ...
    expect(screen.getByText('messages.optionNumber')).toBeInTheDocument();
  ...
  it('names generic allow and reject answers in the reader language and keeps specific wording', () => {
    const message = makeAcpMessage();
    message.content.options = [
      // The mu bridge's confirmation: stable ids, English fallback names.
      { option_id: 'allow', name: 'Allow', kind: 'allow_once' },
      { option_id: 'reject', name: 'Reject', kind: 'reject_once' },
      { option_id: 'always', name: 'Always Allow', kind: 'allow_always' },
      // A harness choice or an agent's specific answer stays as sent.
      { option_id: '0', name: 'Keep the old checkpoint', kind: 'allow_once' },
      { option_id: 'no-kind', name: 'Allow', kind: '' as never },
    ];
    render(<MessageAcpPermission message={message} />);

    expect(screen.getByTestId('message-acp-permission-option-allow')).toHaveTextContent(
      'messages.permissionOption.allow_once'
    );
    expect(screen.getByTestId('message-acp-permission-option-reject')).toHaveTextContent(
      'messages.permissionOption.reject_once'
    );
    expect(screen.getByTestId('message-acp-permission-option-always')).toHaveTextContent(
      'messages.permissionOption.allow_always'
    );
    expect(screen.getByTestId('message-acp-permission-option-0')).toHaveTextContent('Keep the old checkpoint');
    expect(screen.getByTestId('message-acp-permission-option-no-kind')).toHaveTextContent('Allow');
  });
});
