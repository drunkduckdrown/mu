...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    // A failure detail is echoed after the key, so a test can see which sentence carries which detail.
    t: (key: string, options?: { defaultValue?: string; error?: string }) =>
      options?.defaultValue || (options?.error === undefined ? key : `${key}(${options.error})`),
    i18n: { language: 'en-US' },
  }),
}));
...
describe('TeamAddMemberPopover', () => {
  ...
  it('keeps the popover content available and does not switch tabs on add failure', async () => {
    ...
    // A translated headline for the action, the backend's own text only as its detail.
    expect(messageErrorMock).toHaveBeenCalledWith('team.addMember.addFailedWithDetail(failed)');
    expect(screen.getAllByTestId('team-add-member-option-writer')).toHaveLength(2);
