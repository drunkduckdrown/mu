...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string, options?: { defaultValue?: string; model?: string; level?: string }) => {
...
describe('AcpModelSelector runtime options', () => {
  ...
  it('names pi thinking levels reported as raw ids and keeps any other agent name', () => {
    ...
    render(<AcpModelSelector conversation_id='conversation-1' backend='kyrn' />);

    expect(screen.getByTestId('acp-model-selector')).toHaveTextContent('GPT-5.2 · Very high');
    const thoughtBody = screen.getAllByTestId('submenu-body')[1];
    expect(within(thoughtBody).getByText('Very high')).toBeInTheDocument();
    expect(within(thoughtBody).getByText('Turbo mode')).toBeInTheDocument();
