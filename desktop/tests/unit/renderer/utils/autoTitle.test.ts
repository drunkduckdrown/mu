...
describe('autoTitle', () => {
  ...
  describe('deriveAutoTitleFromMessages', () => {
    ...
    it('skips slash commands, in history and as the fallback', () => {
      const messages = [mockUserMessage('/board on'), mockUserMessage('Fix the login page')];
      expect(deriveAutoTitleFromMessages(messages)).toBe('Fix the login page');
      expect(deriveAutoTitleFromMessages([mockUserMessage('  /help')], '/board off')).toBeNull();
      expect(deriveAutoTitleFromMessages([mockUserMessage('/board on')], 'Fix the login page')).toBe(
        'Fix the login page'
      );
    });
