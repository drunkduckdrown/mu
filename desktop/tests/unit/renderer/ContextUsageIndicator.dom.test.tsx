...
// The locale's own sentences by full key; a key missing here falls back to the component's English default.
let mockTranslations: Record<string, string> = {};

vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string, fallback?: string, options?: Record<string, unknown>) => {
      const text = mockTranslations[key] ?? fallback ?? key;
...
describe('ContextUsageIndicator', () => {
  ...
  it("words every popover line as one sentence of the app language, numbers inside, in that language's order", () => {
    mockLanguage = 'zh-CN';
    mockTranslations = Object.fromEntries(
      Object.entries(zhCNConversation.contextUsage).map(([name, text]) => [`conversation.contextUsage.${name}`, text])
    );
    const { getByTestId } = render(
      <ContextUsageIndicator
        tokenUsage={{
          total_tokens: 12_600,
          cost: { amount: 0.42, currency: 'USD' },
          breakdown: { input_tokens: 14_088, output_tokens: 30, cached_read_tokens: 14_080, cached_write_tokens: 512 },
        }}
        context_limit={262_144}
      />
    );

    const lines = Array.from(getByTestId('popover-content').querySelectorAll(':scope > div > div')).map(
      (line) => line.textContent
    );
    // Chinese puts "上下文已使用" before the figures; English says "… context used" after them.
    expect(lines[0]).toBe('上下文已使用 4.8% · 12.6K / 262.1K');
    expect(lines[1]).toBe('会话成本 ≈ US$0.42');
    expect(lines[2]).toBe('输入 14.1K · 输出 30 · 缓存读取 14.1K · 缓存写入 512');
  });
