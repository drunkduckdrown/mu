...
describe('useAcpMessage', () => {
  ...
  it('keeps thought history when an ACP error ends the turn', async () => {
    vi.mocked(getConversationOrNull).mockResolvedValue(null);
    ...
    act(() => {
      responseStreamHandlerRef.current?.({
        type: 'thinking',
        data: { content: 'checking input', status: 'thinking' },
        msg_id: 'thought-1',
        conversation_id: 'conv-1',
        created_at: 1_000,
      });
    ...
    expect(addOrUpdateMessageMock).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        type: 'thinking',
        msg_id: 'thought-1',
        content: expect.objectContaining({ status: 'done', duration: 1_000 }),
      })
    );
    expect(addOrUpdateMessageMock).toHaveBeenLastCalledWith(
      expect.objectContaining({ type: 'tips', content: expect.objectContaining({ content: 'request failed' }) })
    );
  });
