...
describe('formatManagedAgentDiagnosticMessage', () => {
  ...
  it("puts the version numbers after the explanation in the language's own brackets", () => {
    const bracketT = ((key: string, options?: Record<string, unknown>) => {
      if (key === 'settings.agentManagement.errorCodes.version_drift_newer') return 'Newer than verified';
      if (key === 'settings.agentManagement.versionDriftWithVersions') {
        return `${String(options?.explanation)} (${String(options?.versions)})`;
      }
      return String(options?.defaultValue ?? key);
    }) as unknown as TFunction;
    ...
    const message = formatManagedAgentDiagnosticMessage(
      bracketT,
      managedAgent({ last_check_error_code: 'version_drift_newer', last_check_error_message: '1.2.0 → 1.3.0' })
    );

    expect(message).toBe('Newer than verified (1.2.0 → 1.3.0)');
