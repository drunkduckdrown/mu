...
vi.mock('@/renderer/hooks/system/useSpeechInput', () => ({
  getSpeechInputErrorMessageKey: vi.fn(() => 'conversation.chat.speech.transcriptionFailed'),
  useSpeechInput: () => ({
    availability: 'record',
    clearError: vi.fn(),
...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string, options?: { shortcut?: string; status?: string }) =>
...
describe('SpeechInputButton', () => {
  ...
  it('puts the HTTP status of a failed transcription into the translated message, not the English status text', async () => {
    mocks.errorCode = 'transcription-failed';
    mocks.errorMessage = '401 Unauthorized';
    render(<SpeechInputButton onTranscript={vi.fn()} />);

    await waitFor(() =>
      expect(Message.error).toHaveBeenCalledWith('conversation.chat.speech.transcriptionFailedStatus:401')
    );
    expect(vi.mocked(Message.error).mock.calls.flat().join(' ')).not.toContain('Unauthorized');
  });

  it('shows the plain failure message when there is no status', async () => {
    mocks.errorCode = 'transcription-failed';
    render(<SpeechInputButton onTranscript={vi.fn()} />);

    await waitFor(() => expect(Message.error).toHaveBeenCalledWith('conversation.chat.speech.transcriptionFailed'));
  });
