...
describe('outcome reporting', () => {
  ...
  it('lists the files that did not complete the way the app language joins a list', async () => {
    install({
      resources: [
        resource('a.ts', { staged: false }),
        resource('b.ts', { staged: false }),
        resource('c.ts', { staged: false }),
      ],
      failures: [
        { file: { pe_id: 'pe1', relative_path: 'a.ts' }, reason: 'io' },
        { file: { pe_id: 'pe1', relative_path: 'b.ts' }, reason: 'io' },
      ],
    });
    render(<ScmPanel projectId='p1' />);
    await screen.findByText('a.ts');
    ...
    fireEvent.click(document.querySelector('[data-scm-header-stage-all]')!);
    await waitFor(() => expect(document.querySelector('[data-scm-report="warning"]')).not.toBeNull());
    expect(document.body.textContent).toContain('actions.partialDetail:{"files":"a.ts and b.ts"}');
  });
