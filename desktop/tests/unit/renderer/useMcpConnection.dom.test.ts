...
// Echoes the key and its options, so a test sees which sentence was asked for and with what.
...
describe('useMcpConnection — ELECTRON-1A1 unmount race', () => {
  ...
  it('still shows the success message when the context is alive', async () => {
    ...
    await waitFor(() => {
      expect(success).toHaveBeenCalledWith('settings.mcpTestConnectionSuccessNamed {"name":"My Server"}');
    });
  });

  it('names the server inside one translated sentence when it needs a sign-in', async () => {
    vi.mocked(mcpService.testMcpConnection.invoke).mockResolvedValue({ needsAuth: true } as never);
    const warning = vi.fn();
    const message = { warning, success: vi.fn(), error: vi.fn() } as unknown as Parameters<typeof useMcpConnection>[1];
    ...
    await waitFor(() => {
      expect(warning).toHaveBeenCalledWith('settings.mcpAuthRequiredNamed {"name":"My Server"}');
    });
  });

  it('puts the failure into the translated hint without a second full stop', async () => {
    vi.mocked(mcpService.testMcpConnection.invoke).mockResolvedValue({
      success: false,
    ...
    await waitFor(() => {
      expect(error).toHaveBeenCalledWith({
        content: 'settings.mcpTestConnectionFailedWithHint {"name":"My Server","error":"Server closed the stream"}',
        duration: 5000,
      });
    });
  });

  it('keeps the mark of a cut-off failure when it drops the final full stop', async () => {
    const longFailure = `${'x'.repeat(160)}.`;
    vi.mocked(mcpService.testMcpConnection.invoke).mockResolvedValue({ success: false, error: longFailure } as never);
    const error = vi.fn();
    const message = { warning: vi.fn(), success: vi.fn(), error } as unknown as Parameters<typeof useMcpConnection>[1];
    ...
    await waitFor(() => {
      expect(error).toHaveBeenCalledWith({
        content: `settings.mcpTestConnectionFailedWithHint {"name":"My Server","error":"${'x'.repeat(150)}…"}`,
        duration: 5000,
      });
    });
  });
