...
describe('TeamCreateModal', () => {
  ...
  it('model_resolution_failure_blocks_create_and_names_assistant', async () => {
    ...
    // One translated sentence names the member and carries the failure as its detail; it is not wrapped again in the
    // "couldn't create the team" headline.
    expect(messageErrorMock).toHaveBeenCalledWith('team.create.memberErrorWithDetail');
    expect(tMock).toHaveBeenCalledWith('team.create.memberErrorWithDetail', {
      name: 'Remote Runner',
      error: 'model unavailable',
    });
  });

  it('leads a backend failure with the translated team headline, the raw text as its detail', async () => {
    resolveDefaultTeamAgentModelMock.mockResolvedValue('model-ok');
    createTeamInvokeMock.mockRejectedValueOnce(new Error('disk full'));
    render(<TeamCreateModal visible onClose={vi.fn()} onCreated={vi.fn()} />);

    fireEvent.change(screen.getByPlaceholderText('Team name'), {
      target: { value: 'Backend Failure Team' },
    });
    fireEvent.click(screen.getByTestId('team-create-agent-option-bare-aionrs'));
    fireEvent.click(screen.getByRole('button', { name: 'Confirm Create' }));

    await waitFor(() => expect(messageErrorMock).toHaveBeenCalledWith('team.create.errorWithDetail'));
    expect(tMock).toHaveBeenCalledWith('team.create.errorWithDetail', { error: 'disk full' });
  });
});
