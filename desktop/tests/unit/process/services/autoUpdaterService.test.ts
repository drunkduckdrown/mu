...
describe('AutoUpdaterService', () => {
  ...
  it('sends a reason code with each auto-updater error, for the renderer to translate', async () => {
    appMock.isPackaged = true;
    const { autoUpdaterService } = await import('@/process/services/autoUpdaterService');
    autoUpdaterService.initialize();

    const statuses: Array<{ status: string; error?: string; errorInfo?: { code: string } }> = [];
    autoUpdaterService.on('update-status', (s: (typeof statuses)[number]) => statuses.push(s));

    getErrorHandler()(new Error('net::ERR_INTERNET_DISCONNECTED'));

    expect(statuses.find((s) => s.status === 'error')).toEqual({
      status: 'error',
