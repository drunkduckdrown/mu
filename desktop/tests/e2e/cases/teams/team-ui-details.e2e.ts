...
test.describe('Team UI Details', () => {
  test('collapsed sidebar shows team icon and navigates on click', async ({ page }) => {
    ...
    const collapseBtn = page.locator('button[aria-label="Collapse sidebar"], button[aria-label="收起侧边栏"]');
  ...
  test('create team with workspace folder via native dialog', async ({ electronApp, page }) => {
    ...
    if (menuVisible) {
      const browseOption = menu.locator('text=Choose a different folder').or(menu.locator('text=选择其他目录'));
