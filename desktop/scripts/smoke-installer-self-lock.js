...
function main() {
  ...
  const nsi = `
Unicode true
Name "mu Installer Self Lock Smoke"
...
`;
  ...
  try {
    ...
    if (!blocking.some((process) => process.name === 'mu installer' && Number(process.pid) > 0)) {
      throw new Error(`expected mu installer blocker, got ${JSON.stringify(blocking)}`);
    }
