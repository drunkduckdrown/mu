...
/**
 * The i18n key naming a built-in theme, so its card and the "applied" toast follow the app language; `name` above is
 * only the stored fallback. Undefined for extension and user themes, whose names are their authors'.
 */
export function builtinThemeNameKey(id: string): 'settings.lightMode' | 'settings.darkMode' | undefined {
  if (id === LIGHT_THEME_ID) return 'settings.lightMode';
  if (id === DARK_THEME_ID) return 'settings.darkMode';
  return undefined;
}
