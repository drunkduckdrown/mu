...
i18n
  ...
  .init({
    ...
    // The upstream strings name the product AionUi: shown as mu (see common/kyrn/displayName.ts).
...
ipcBridge.systemSettings.languageChanged.on(async ({ language }) => {
  ...
  // The change came from elsewhere (another window, the WebUI), so the main process has not heard of it yet: let its
  // menus, tray and the harness follow too. No loop: the main process only applies the language and emits nothing
  // back on this channel, and the early return above stops a repeat of the same language.
  void ipcBridge.systemSettings.syncMainLanguage.invoke({ language: normalized }).catch(() => {});
});
