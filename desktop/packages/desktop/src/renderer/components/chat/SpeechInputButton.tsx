...
const SpeechInputButton: React.FC<SpeechInputButtonProps> = ({ onLiveTranscript, onTranscript }) => {
  ...
  useEffect(() => {
    ...
    // The only detail is the transcription service's HTTP status ("401 Unauthorized"): the number goes into the
    // translated sentence, the English status text is dropped.
    const httpStatus = errorMessage?.trim().match(/^\d{3}\b/)?.[0];
    Message.error(
      httpStatus ? t('conversation.chat.speech.transcriptionFailedStatus', { status: httpStatus }) : baseMessage
    );
    clearError();
