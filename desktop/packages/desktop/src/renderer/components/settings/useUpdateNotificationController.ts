...
export const useUpdateNotificationController = () => {
  ...
  const checkForUpdates = useCallback(async () => {
    ...
    switch (outcome.kind) {
      ...
      case 'upToDate':
        ...
        return;
      case 'error':
        dispatch({ type: 'checkError', message: describeUpdateError(t, outcome.error, 'update.checkFailed') });
        return;
    }
  }, [t]);
    ...
    const removeListener = ipcBridge.autoUpdate.status.on((evt: AutoUpdateStatus) => {
      ...
      switch (evt.status) {
        ...
        case 'preparing-install':
          ...
          break;
        case 'error':
          if (evt.error) console.warn('Auto-update error:', evt.error);
          dispatch({ type: 'autoError', message: describeUpdateError(t, evt.errorInfo, 'update.downloadFailed') });
          break;
        case 'checking':
        case 'not-available':
        case 'cancelled':
  ...
  useEffect(() => {
    const removeProgressListener = ipcBridge.update.downloadProgress.on((evt: UpdateDownloadProgressEvent) => {
      if (!evt) return;
      if (evt.status === 'error' && evt.error) console.warn('Update download failed:', evt.error);
      dispatch({
        type: 'manualProgress',
        ...
        error: describeUpdateError(t, evt.errorInfo, 'update.downloadFailed'),
      });
    });
  ...
  const startAutoDownload = useCallback(async () => {
    ...
    if (!res?.success) {
      if (res?.msg) console.warn('Auto-update download failed to start:', res.msg);
      dispatch({ type: 'autoError', message: describeUpdateError(t, res?.errorInfo, 'update.downloadStartFailed') });
    }
  }, [t]);
  ...
  const startManualInstallDownload = useCallback(async () => {
    ...
    if (!res?.success || !res.data) {
      if (res?.msg) console.warn('Update download failed to start:', res.msg);
      dispatch({ type: 'checkError', message: describeUpdateError(t, res?.errorInfo, 'update.downloadStartFailed') });
      return;
    }
    dispatch({
  ...
  const cancelDownload = useCallback(() => {
    ...
    void cancelTask(task).catch((error) => {
      console.warn('Update download could not be cancelled:', error);
      dispatch({ type: 'autoError', message: t('update.errorTitle') });
    });
  }, [cancelTask, t]);
