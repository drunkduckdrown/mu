...
const ChannelHeader: React.FC<ChannelHeaderProps> = ({ channel, onToggleEnabled }) => {
  ...
  const logoSrc = channelLogoMap[channel.id] || resolveExtensionAssetUrl(channel.icon);
  ...
  return (
    <div className='flex items-center justify-between group' data-channel-header={channel.id}>
      <div className='flex items-center gap-8px flex-1 min-w-0'>
        {/* Decorative: the channel's title, in the app language, sits right next to it. */}
        {logoSrc && <img src={logoSrc} alt='' className='w-14px h-14px object-contain shrink-0' />}
