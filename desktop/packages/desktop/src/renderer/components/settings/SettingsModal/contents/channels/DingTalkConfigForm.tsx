...
const DingTalkConfigForm: React.FC<DingTalkConfigFormProps> = ({ pluginStatus, modelSelection, onStatusChange }) => {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        Message.error(
          result.error
            ? t('settings.assistant.connectionFailedWithDetail', { error: result.error })
            : t('settings.dingtalk.connectionFailed', 'Connection failed')
        );
      }
    } catch (error: any) {
      setCredentialsTested(false);
      Message.error(
        error?.message
          ? t('settings.assistant.connectionFailedWithDetail', { error: error.message })
          : t('settings.dingtalk.connectionFailed', 'Connection failed')
      );
    } finally {
  ...
  const handleAutoEnable = async () => {
    try {
      ...
      Message.error(
        detail
          ? t('settings.assistant.enableFailedWithDetail', { error: detail })
  ...
  const handleApprovePairing = async (code: string) => {
    try {
      ...
      Message.error(t('settings.assistant.approveFailedWithDetail', { error: parseError(error) }));
    }
  };
  ...
  const handleRejectPairing = async (code: string) => {
    try {
      ...
      Message.error(t('settings.assistant.rejectFailedWithDetail', { error: parseError(error) }));
    }
  };
  ...
  const handleRevokeUser = async (user_id: string) => {
    try {
      ...
      Message.error(t('settings.assistant.revokeFailedWithDetail', { error: parseError(error) }));
    }
  };
  ...
  // The console link inside the credential descriptions; its text comes from the translated sentence.
  const devConsoleLink = (
    <a
  ...
  return (
    <div className='flex flex-col gap-24px'>
      ...
      <PreferenceRow
        ...
        description={
          <span>
            <Trans t={t} i18nKey='settings.dingtalk.clientIdDesc' components={[devConsoleLink]} />
          </span>
        }
        required
      >
      ...
      <PreferenceRow
        ...
        description={
          <span>
            <Trans t={t} i18nKey='settings.dingtalk.clientSecretDesc' components={[devConsoleLink]} />
          </span>
        }
        required
      >
        ...
        >
          ...
          {pluginStatus?.connected && (
            <div className='text-14px text-t-secondary space-y-8px'>
              <p className='m-0 font-500'>{t('settings.assistant.nextStepsLabel')}</p>
              <p className='m-0'>
                ...
                <div key={pairing.code} className='flex items-center justify-between bg-fill-2 rd-8px p-12px'>
                  <div className='flex-1'>
                    ...
                    <div className='text-12px text-t-tertiary mt-4px'>
                      {t('settings.assistant.pairingCodeLabel')}{' '}
                ...
                <div key={user.id} className='flex items-center justify-between bg-fill-2 rd-8px p-12px'>
                  <div className='flex-1'>
                    <div className='text-14px font-500 text-t-primary'>
                      {user.display_name || t('settings.assistant.unknownUser')}
                    </div>
                    <div className='text-12px text-t-tertiary mt-4px'>
                      {t('settings.assistant.platformValue', {
                        platform: t(`settings.channels.${user.platformType}Title`, { defaultValue: user.platformType }),
                      })}
                      <span className='mx-8px'>|</span>
                      {t('settings.assistant.authorizedAtValue', { time: formatTime(user.authorizedAt) })}
                    </div>
                  </div>
                  <Tooltip content={t('settings.assistant.revokeAccess', 'Revoke access')}>
                    <Button
                      ...
                      size='small'
                      icon={<Delete size={16} />}
                      aria-label={t('settings.assistant.revokeAccess')}
