...
const WeixinConfigForm: React.FC<WeixinConfigFormProps> = ({ pluginStatus, modelSelection, onStatusChange }) => {
  ...
  const handleDisconnect = async () => {
    try {
      ...
      Message.error(t('settings.assistant.disableFailedWithDetail', { error: parseError(error) }));
    }
  };
              ...
              {authorizedUsers.map((user) => (
                <div key={user.id} className='flex items-center justify-between bg-fill-2 rd-8px p-12px'>
                  <div className='flex-1'>
                    ...
                    <div className='text-12px text-t-tertiary mt-4px'>
                      {t('settings.assistant.authorizedAtValue', {
                        time: formatDateTime(user.authorizedAt, i18n.language),
                      })}
                    </div>
                  </div>
