...
/** Channel logos in the Channels tab title; each is named by the channel's title in the app language. */
const CHANNEL_LOGOS = [
  { id: 'telegram', src: ChannelTelegramLogo, titleKey: 'settings.channels.telegramTitle' },
  { id: 'lark', src: ChannelLarkLogo, titleKey: 'settings.channels.larkTitle' },
  { id: 'dingtalk', src: ChannelDingTalkLogo, titleKey: 'settings.channels.dingtalkTitle' },
  { id: 'weixin', src: ChannelWeixinLogo, titleKey: 'settings.channels.weixinTitle' },
  { id: 'wecom', src: ChannelWecomLogo, titleKey: 'settings.channels.wecomTitle' },
  { id: 'slack', src: ChannelSlackLogo, titleKey: 'settings.channels.slackTitle' },
  { id: 'discord', src: ChannelDiscordLogo, titleKey: 'settings.channels.discordTitle' },
] as const;
...
const WebuiModalContent: React.FC = () => {
  ...
  const handleSetNewPassword = async () => {
    try {
      ...
      const rawMsg =
        isBackendHttpError(error) && error.backendMessage
          ...
          : error instanceof Error
            ? error.message
            : '';
      // The backend sends its password rule codes joined with '; '. Each known one becomes a sentence of its own
      // line; anything else is shown as the detail under a translated headline.
      const translated = rawMsg
        .split(';')
        .map((code) => errorCodeMap[code.trim()])
        .filter(Boolean);
      if (translated.length > 0) {
        ...
        Message.error(
          rawMsg
            ? t('settings.webuiAccount.passwordChangeFailedWithDetail', { error: rawMsg })
            : t('settings.webui.passwordChangeFailed')
        );
      }
    } finally {
  ...
  const handleSetNewUsername = async () => {
    try {
      ...
      Message.error(
        detail
          ? t('settings.webuiAccount.usernameChangeFailedWithDetail', { error: detail })
          : t('settings.webui.usernameChangeFailed')
      );
    } finally {
                ...
                <div className='p-12px bg-fill-1 border border-line rd-10px'>
                  {qrLoading ? (
                    ...
                    <div className='p-8px bg-white rd-8px'>
                      ...
                      >
                        <QRCodeSVGLazy value={qrUrl} size={140} level='M' title={t('settings.webui.qrLogin')} />
                      </Suspense>
                    </div>
                  ) : (
                    <div className='w-140px h-140px flex items-center justify-center'>
            ...
            >
              ...
              <span className='inline-flex items-center gap-4px ms-2px'>
                {CHANNEL_LOGOS.map((item) => (
                  ...
                  >
                    <img src={item.src} alt={t(item.titleKey)} className='w-14px h-14px object-contain' />
                  </span>
                ))}
              </span>
            </span>
