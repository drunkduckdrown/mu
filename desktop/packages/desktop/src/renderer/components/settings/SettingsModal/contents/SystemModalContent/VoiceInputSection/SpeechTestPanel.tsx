...
const SpeechTestPanel: React.FC<SpeechTestPanelProps> = ({ config, source }) => {
  ...
  const handleTranscript = useCallback((transcript: string) => {
    setResult({ text: transcript, elapsedMs: Date.now() - startedAtRef.current });
  }, []);
    ...
    <div className='mt-16px flex flex-col gap-8px'>
      ...
      {errorCode && (
        <Alert
          ...
          content={
            <div>
              <div>{t(getSpeechInputErrorMessageKey(errorCode))}</div>
              {/* The service's own reason, untranslated: a secondary detail under the headline. */}
              {errorMessage?.trim() && (
                <div className='mt-4px text-12px text-t-secondary break-all'>{errorMessage.trim()}</div>
              )}
            </div>
          }
        />
      ...
      {result && (
        <Alert
          type='success'
          content={t('settings.speechToTextTestResultLine', {
            elapsed: formatNumber(result.elapsedMs / 1000, i18n.language, {
              style: 'unit',
              unit: 'second',
              unitDisplay: 'short',
