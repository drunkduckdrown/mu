...
const ChannelModalContent: React.FC = () => {
  ...
  const handleTogglePlugin = async (enabled: boolean) => {
    ...
    try {
      ...
      Message.error(
        t(enabled ? 'settings.assistant.enableFailedWithDetail' : 'settings.assistant.disableFailedWithDetail', {
        ...
        <div className='space-y-10px py-4px'>
          ...
          {pluginType === 'ext-wecom-bot' && (
            <div className='text-12px leading-relaxed p-10px rd-8px bg-[rgba(var(--orange-6),0.08)] border border-[rgba(var(--orange-6),0.3)] text-t-secondary'>
              ...
              <div>{t('settings.wecom.callbackUrlLocal', { url: localCallbackUrl })}</div>
              {lanCallbackUrl ? <div>{t('settings.wecom.callbackUrlLan', { url: lanCallbackUrl })}</div> : null}
              {publicCallbackUrl ? (
                <div>{t('settings.wecom.callbackUrlPublic', { url: publicCallbackUrl })}</div>
              ) : null}
