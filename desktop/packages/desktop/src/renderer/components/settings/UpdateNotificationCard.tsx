...
const formatPercent = (percent: number, language: string) =>
  formatNumber(percent / 100, language, { style: 'percent', maximumFractionDigits: 0 });
  ...
  if (state.presentation === 'mini') {
    ...
    const miniContent =
      state.status === 'downloaded' ? (
        ...
        <span className='text-13px leading-none text-t-primary font-600'>
          {formatPercent(miniPercent, i18n.language)}
        </span>
      );
    ...
    return (
      <div className='py-8px'>
        ...
        <div className='flex justify-between gap-12px text-12px text-t-tertiary'>
          <span>{formatPercent(percent, i18n.language)}</span>
          <span>
