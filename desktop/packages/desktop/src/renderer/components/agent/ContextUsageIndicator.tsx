...
const ContextUsageIndicator: React.FC<ContextUsageIndicatorProps> = ({
  ...
  // Each counter is one translated phrase with its number inside ("Input 14.1K"), so the word order is the language's.
  const breakdown = tokenUsage.breakdown;
  const breakdownParts: string[] = [];
  if (breakdown) {
    if (typeof breakdown.input_tokens === 'number') {
      breakdownParts.push(
        t('conversation.contextUsage.inputTokens', 'Input {{tokens}}', {
          tokens: formatTokenCount(breakdown.input_tokens, locale),
        })
      );
    }
    if (typeof breakdown.output_tokens === 'number') {
      breakdownParts.push(
        t('conversation.contextUsage.outputTokens', 'Output {{tokens}}', {
          tokens: formatTokenCount(breakdown.output_tokens, locale),
        })
      );
    }
    if (breakdown.cached_read_tokens) {
      breakdownParts.push(
        t('conversation.contextUsage.cachedReadTokens', 'Cache read {{tokens}}', {
          tokens: formatTokenCount(breakdown.cached_read_tokens, locale),
        })
      );
    }
    if (breakdown.cached_write_tokens) {
      breakdownParts.push(
        t('conversation.contextUsage.cachedWriteTokens', 'Cache write {{tokens}}', {
          tokens: formatTokenCount(breakdown.cached_write_tokens, locale),
        })
      );
    }
    if (breakdown.thought_tokens) {
      breakdownParts.push(
        t('conversation.contextUsage.thoughtTokens', 'Thinking {{tokens}}', {
          tokens: formatTokenCount(breakdown.thought_tokens, locale),
        })
      );
    }
  }
    ...
    <>
      {tokenUsage.cost && (
        <div className='text-12px text-t-secondary mt-4px'>
          {t('conversation.contextUsage.sessionCostApprox', 'Session cost ≈ {{cost}}', {
            cost: formatCostAmount(tokenUsage.cost, locale),
          })}
        </div>
      )}
  ...
  const popoverContent = hasWindow ? (
    <div className='p-8px min-w-160px'>
      <div className='text-14px font-medium text-t-primary'>
        {t('conversation.contextUsage.usedOfWindow', '{{percent}} · {{used}} / {{limit}} context used', {
          percent: formatPercentage(percentage, locale),
          used: displayTotal,
          limit: displayLimit,
        })}
      </div>
