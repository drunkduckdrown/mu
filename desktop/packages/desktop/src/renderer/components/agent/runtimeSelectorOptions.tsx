...
import type { AcpConfigSetStatus } from '@/renderer/hooks/agent/useAcpConfigOptions';
...
/** A thinking-level option list as the agent reported it (ACP config options or the runtime catalog). */
type ThoughtLevelOptions = {
  currentValue?: string | null;
  options: ReadonlyArray<{ value: string; label?: string | null }>;
};

/**
 * A thinking-level option's name in the reader's language. Agents report pi's levels by id ("xhigh"), which is no
 * word in any language, so those are named by the app; any other value keeps the agent's own name.
 */
export const thoughtLevelOptionLabel = (t: TFunction, option: { value: string; label?: string | null }): string =>
  isThinkingLevel(option.value) ? thinkingLevelLabel(t, option.value) : option.label || option.value;

export const getCurrentThoughtLevelLabel = (
  t: TFunction,
  thoughtLevel: ThoughtLevelOptions | null | undefined
): string => {
  if (!thoughtLevel?.currentValue) return '';
  const current = thoughtLevel.options.find((item) => item.value === thoughtLevel.currentValue);
  return thoughtLevelOptionLabel(t, current ?? { value: thoughtLevel.currentValue });
};
...
export const composeRuntimeSelectorLabel = ({
  ...
  t: TFunction;
  modelLabel: string;
  thoughtLevel?: ThoughtLevelOptions | null;
}): string => {
  const thoughtLevelLabel = getCurrentThoughtLevelLabel(t, thoughtLevel);
  if (!thoughtLevelLabel) return modelLabel;
  return t('agent.model.withThoughtLevel', { model: modelLabel, level: thoughtLevelLabel });
};
