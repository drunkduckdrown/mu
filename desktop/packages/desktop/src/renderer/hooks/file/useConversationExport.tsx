...
type UseConversationExportOptions = {
  ...
  /** The app language (`i18n.language`), for the export time in the transcript header. */
  language?: string;
...
export function useConversationExport(options: UseConversationExportOptions): UseConversationExportResult {
  const { conversation_id, workspace, t, language, messageApi } = options;
  ...
  const transcriptLabels = useMemo<ExportTranscriptLabels>(
    () => ({
      ...
      language,
      headerLine: (label: string, value: string) => t('messages.exportHeaderLine', { label, value }),
      speakerLine: (speaker: string) => t('messages.exportSpeakerLine', { speaker }),
    }),
    [language, t]
  );
