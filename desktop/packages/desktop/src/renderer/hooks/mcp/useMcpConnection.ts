...
/**
 * 截断过长的错误消息，保持可读性
 * Truncate long error messages to keep them readable. The cut is marked with an ellipsis character ("…"), not three
 * full stops, so asFailureDetail below does not take it for the sentence's own full stop and drop it.
 */
...
/**
 * The failure as the detail of `settings.mcpTestConnectionFailedWithHint`, whose translation ends the detail with its
 * own full stop: the detail's own final full stop is dropped so the sentence does not end in two.
 */
const asFailureDetail = (message: string): string => message.trim().replace(/[.。．]+$/u, '');
