...
interface ValidationResult {
  isValid: boolean;
  errorMessage?: string;
  /** The JSON parser's own message (English, engine-specific), shown under the translated headline. */
  errorDetail?: string;
}
  ...
  const validateJsonSyntax = useCallback(
    (input: string): ValidationResult => {
      ...
      try {
        ...
        return {
          isValid: false,
          errorMessage: t('settings.mcpJsonFormatError'),
          errorDetail: error instanceof SyntaxError ? error.message : undefined,
        };
      }
    },
    [t]
        ...
        <div>
          ...
          {!validation.isValid && jsonInput.trim() && (
            <Alert
              ...
              content={
                <>
                  <div>{validation.errorMessage || t('settings.mcpJsonFormatError')}</div>
                  {validation.errorDetail ? (
                    <div className='mt-2px text-12px text-t-secondary' dir='ltr'>
                      {validation.errorDetail}
                    </div>
                  ) : null}
                </>
