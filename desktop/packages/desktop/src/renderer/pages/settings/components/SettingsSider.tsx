...
export const LEGACY_ANCHOR_REMAP: Record<string, string> = {
  ...
  capabilities: 'skills',
  display: 'appearance',
  // mu's settings were one tab before they became one entry per section.
  kyrn: 'mu-models',
};
...
const GROUP_HEADER_BEFORE: Record<string, string> = {
  'mu-models': 'settings.groupAiCore',
    ...
    const builtinMap: Record<string, SiderItem> = {
      ...Object.fromEntries(
        MU_TABS.map(({ id, section, Icon }) => [
          id,
          { id, label: t(`mu.sections.${section}`), icon: <Icon />, path: muTabPath(section) },
        ])
      ),
