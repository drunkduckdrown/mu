...
import { buildSkillImportNotice, formatSkillSize, getSkillImportErrorMessage } from './skillImportMessages';
...
const SkillsHubSettings: React.FC<SkillsHubSettingsProps> = ({ withWrapper = true }) => {
  ...
  const handleImport = async (skillPath: string) => {
    try {
      ...
      const notice = buildSkillImportNotice(result, t, i18n.language);
  ...
  const handleManualImport = async () => {
    try {
      const result = await ipcBridge.dialog.showOpen.invoke({
        ...
        filters: [{ name: t('settings.skillsHub.importFilterName'), extensions: ['zip'] }],
      });
  ...
  const getFailedImportDescription = (record: SkillImportRecord) => {
    const actual = formatSkillSize(record.actual_bytes, i18n.language);
    const limit = formatSkillSize(record.limit_bytes, i18n.language);
    switch (record.error_code) {
      case 'SKILL_IMPORT_FILE_TOO_LARGE':
      ...
      <div className='flex items-start justify-between gap-12px'>
        <p className='m-0 flex-1 min-w-0 text-12px leading-relaxed text-t-tertiary'>
          {t('settings.skillsHub.customHint', {
            maxFileSize:
              formatSkillSize(importLimits?.max_file_bytes, i18n.language) ??
              t('settings.skillsHub.importHelpConfiguredLimit', { defaultValue: 'configured limit' }),
            maxTotalSize:
              formatSkillSize(importLimits?.max_total_bytes, i18n.language) ??
              t('settings.skillsHub.importHelpConfiguredLimit', { defaultValue: 'configured limit' }),
            defaultValue:
