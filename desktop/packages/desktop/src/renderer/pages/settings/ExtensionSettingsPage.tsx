...
/** Why no tab can be shown; rendered in the app language at display time. */
type TabError = { code: 'noTabId' } | { code: 'notFound'; id: string };
...
const ExtensionSettingsPage: React.FC = () => {
  ...
  const { tab, error } = useMemo<{ tab: IExtensionSettingsTab | null; error: TabError | null }>(() => {
    if (!tabId) {
      return { tab: null, error: { code: 'noTabId' } };
    }
    ...
    return { tab: null, error: { code: 'notFound', id: tabId } };
  }, [tabId, extensionTabs]);
      ...
      <div className='relative w-full h-full min-h-400px'>
        ...
        {error && (
          <div className='flex items-center justify-center h-full text-t-secondary text-14px'>
            {error.code === 'notFound'
              ? t('settings.extensionTab.notFound', { id: error.id })
              : t('settings.extensionTab.noTabId')}
          </div>
        )}
        {tab &&
          (isExternalTab ? (
            ...
            <>
              ...
              <iframe
                ...
                title={t('settings.extensionTab.frameTitle', { name: resolveExtTabName(tab) })}
              />
            </>
          ))}
      </div>
