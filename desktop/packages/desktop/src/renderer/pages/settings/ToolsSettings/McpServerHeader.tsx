...
const getStatusPopoverContent = (
  ...
  if (server.last_test_status === 'connected') {
    ...
    return (
      <div className='max-w-300px space-y-2 text-13px leading-20px'>
        <div className='font-medium text-t-primary'>{t('settings.mcpCheckPassedSummary')}</div>
        {checkedAt ? (
          <div className='text-12px leading-18px text-t-secondary'>
            {t('settings.mcpCheckedAt', { time: checkedAt })}
          </div>
        ) : null}
        <div className='text-12px leading-18px text-t-secondary opacity-80'>{t('settings.mcpCheckPurposeHint')}</div>
      </div>
    );
  }
  ...
  const reasonText =
    server.builtin && server.name === 'chrome-devtools' && server.transport.type === 'stdio'
      ? t('settings.mcpInlineCommandHint', { command: server.transport.command })
      : t('settings.mcpInlineConfigHint');

  return (
    <div className='max-w-300px space-y-2 text-13px leading-20px'>
      <div className='font-medium text-t-primary'>{t('settings.mcpCheckFailedSummary')}</div>
      <div className='text-t-primary'>{reasonText}</div>
      {checkedAt ? (
        <div className='text-12px leading-18px text-t-secondary'>{t('settings.mcpCheckedAt', { time: checkedAt })}</div>
      ) : null}
    </div>
  );
};
...
const getStatusText = (
  server: IMcpServer,
  last_test_status: IMcpServer['last_test_status'] | undefined,
  oauthStatus: McpOAuthStatus | undefined,
  isTestingConnection: boolean | undefined,
  t: (key: string, options?: Record<string, unknown>) => string
) => {
  if (isTestingConnection || last_test_status === 'testing' || oauthStatus?.isChecking) {
  ...
  if (last_test_status === 'error') {
    if (server.builtin && server.name === 'chrome-devtools' && server.transport.type === 'stdio') {
      return t('settings.mcpLocalCommandUnavailable', { command: server.transport.command });
    }
    return t('settings.mcpCheckFailedSimple');
  }

  if (oauthStatus?.needsLogin) {
    return t('settings.mcpNeedsLogin');
  }

  if (last_test_status === 'connected') {
    return t('settings.mcpCheckPassedSimple');
  }

  if (oauthStatus?.isAuthenticated) {
    return t('settings.mcpAuthenticated');
  }
      ...
      {!isReadOnly && (
        <div className='flex items-center gap-2 invisible group-hover:visible' onClick={(e) => e.stopPropagation()}>
          {!server.builtin && (
            ...
            >
              <Button size='mini' icon={<SettingOne size={'14'} />} aria-label={t('common.more')} />
            </Dropdown>
          )}
        </div>
      )}
