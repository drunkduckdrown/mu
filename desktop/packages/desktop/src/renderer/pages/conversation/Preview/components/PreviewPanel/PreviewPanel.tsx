...
import { previewTabDisplayTitle } from '../../context/tabTitle';
...
const PreviewPanel: React.FC = () => {
  ...
  const handleSaveActiveTab = useCallback(async () => {
    ...
    messageApi.error(
      outcome.detail ? t('preview.saveFailedWithDetail', { detail: outcome.detail }) : t('common.saveFailed')
    );
  ...
  const handleSaveAndCloseTab = useCallback(async () => {
    ...
    try {
      ...
      if (outcome.kind === 'conflict') {
        ...
        messageApi.error(detail ? t('preview.saveFailedWithDetail', { detail }) : t('common.saveFailed'));
      }
    }
  ...
  const previewTabs: PreviewTab[] = tabs.map((tab) => {
    ...
    return {
      id: tab.id,
      // Translated here, at render time: a title stored in the language of the moment would not follow a switch.
      title: previewTabDisplayTitle(tab, t),
      isDirty: tab.isDirty,
