...
const PDFPreview: React.FC<PDFPreviewProps> = ({ tabId, fileRef, file_path, content, hideToolbar = false }) => {
  ...
  // The headline is always in the person's language; `detail` is the underlying error's own text, shown smaller.
  ...
  useEffect(() => {
    try {
      ...
      if (!fileRef && !file_path && !content) {
        setError({ message: t('preview.pdf.pathMissing') });
        setLoading(false);
        return;
      }
      ...
      if (webview) {
        ...
        const handleError = () => {
          setError({ message: t('preview.pdf.loadFailed') });
          setLoading(false);
        };
      ...
      setError({ message: t('preview.pdf.loadFailed'), detail: err instanceof Error ? err.message : String(err) });
      setLoading(false);
    }
