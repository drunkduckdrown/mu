...
import { muCodeTheme } from '@renderer/components/Markdown/muCodeTheme';
...
const DiffPreview: React.FC<DiffPreviewProps> = ({
  ...
  const diffHtmlContent = useMemo(() => {
    return html(content, {
      ...
      // diff2html's own English (file tags, the empty-file line) in the reader's language.
      rawTemplates: diff2htmlTemplates(t),
    });
    // `t` is a new function after a language switch, so the templates follow it.
  }, [content, sideBySide, t]);
  ...
  return (
    <div className='flex flex-col w-full h-full overflow-hidden'>
        ...
        <div className='flex items-center justify-between h-40px px-12px bg-bg-2 flex-shrink-0'>
          ...
          <div className='flex items-center gap-8px'>
            {viewMode === 'preview' && (
              ...
              >
                <span className='text-12px text-t-secondary'>{t('preview.sideBySide')}</span>
              </Checkbox>
            )}
            <div
      ...
      <div ref={containerRef} className='flex-1 overflow-auto p-16px'>
        {viewMode === 'source' ? (
          <SyntaxHighlighter
            style={muCodeTheme(currentTheme)}
            language='diff'
            PreTag='div'
            showLineNumbers
            wrapLongLines
      ...
      {viewMode === 'preview' &&
        ...
        ReactDOM.createPortal(
          <Checkbox className='whitespace-nowrap' checked={sideBySide} onChange={(value) => setSideBySide(value)}>
            <span className='whitespace-nowrap'>{t('preview.sideBySide')}</span>
          </Checkbox>,
          operatorRef.current
        )}
