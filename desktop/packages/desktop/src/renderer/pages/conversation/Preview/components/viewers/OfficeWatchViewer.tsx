...
// Accessible names of the web-mode iframe: the document kind, in the reader's language.
const IFRAME_TITLE_KEYS: Record<DocType, string> = {
...
export interface OfficeWatchErrorState {
  code?: OfficeWatchErrorCode;
  /** What the person reads first, always in their language. */
  message: string;
  /** The underlying error's own text (often English), shown smaller under the message. */
  detail?: string;
}
...
/**
 * A start failure without an error code: the headline says it in the person's language, and the error's own text
 * (often an English backend or OS message) only follows as a detail.
 */
export function unmappedStartError(err: unknown, headline: string): OfficeWatchErrorState {
  const detail = err instanceof Error ? err.message.trim() : '';
  return { message: headline, detail: detail && detail !== headline ? detail : undefined };
}
...
const OfficeWatchViewer: React.FC<OfficeWatchViewerProps> = ({ docType, fileRef, file_path, workspace }) => {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        if (!url) {
          // A superseded start (retry, language switch) must not overwrite the newer one's state.
          if (!cancelled) {
        ...
        if (!cancelled) {
          ...
          setError(unmappedStartError(err, t(keys.startFailed)));
          setLoading(false);
        }
      }
    };
    ...
    return (
      <div className='h-full w-full flex items-center justify-center bg-bg-1'>
        <div className='text-center max-w-400px'>
          ...
          {error.detail && (
            <div className='text-12px text-t-tertiary mb-8px break-words select-text'>{error.detail}</div>
          )}
  ...
  return <iframe src={watchUrl} className='w-full h-full border-0 bg-bg-1' title={t(IFRAME_TITLE_KEYS[docType])} />;
};
