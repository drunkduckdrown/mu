...
import { formatDateTime, formatTime } from '@/renderer/services/i18n/format';
...
/**
 * Format a timestamp for message display, in the app language: the time today ("09:05", "09:05 AM"), the date and
 * time on other days, and the year too when it is not this year.
 */
export const formatMessageTime = (timestamp: number, language?: string | null, now: number = Date.now()): string => {
  const date = new Date(timestamp);
  const today = new Date(now);
  const sameYear = date.getFullYear() === today.getFullYear();
  if (sameYear && date.getMonth() === today.getMonth() && date.getDate() === today.getDate()) {
    return formatTime(timestamp, language, { hour: '2-digit', minute: '2-digit' });
  }
