    ...
    <div className='flex flex-col'>
      <div className='flex flex-row color-#86909C gap-12px items-center'>
        <Badge
          ...
          className={normalized.status === 'running' ? styles.breathing : undefined}
        />
        <span
          className={
            'flex-1 min-w-0' +
      ...
      {expanded && hasDetail && (
        <div className={`${styles.detailPanel} m-l-20px m-t-4px`}>
          {normalized.input && (
            <div className={styles.detailSection}>
              <div className={styles.detailLabel}>{t('tools.execution.input')}</div>
              <pre className={styles.detailContent}>{normalized.input}</pre>
            </div>
          )}
          {normalized.output && (
            <div className={styles.detailSection}>
              <div className={styles.detailLabel}>{t('tools.execution.output')}</div>
              <pre className={styles.detailContent}>{normalized.output}</pre>
            </div>
          )}
        </div>
      )}
