...
const StatusTag: React.FC<{ status: string }> = ({ status }) => {
  ...
  const getTagProps = () => {
    switch (status) {
      case 'pending':
        return { color: 'blue', text: t('common.kyrn.toolStatus.pending') };
      case 'in_progress':
        return { color: 'orange', text: t('common.kyrn.toolStatus.in_progress') };
      case 'completed':
        return { color: 'gray', text: t('common.kyrn.toolStatus.completed') };
      case 'failed':
        return { color: 'red', text: t('common.kyrn.toolStatus.failed') };
      default:
        // A status this build does not know is shown as the agent sent it rather than guessed at.
...
/** The card title when the agent sent none: the tool kind in the reader's language. */
const getKindDisplayName = (toolKind: string, t: ReturnType<typeof useTranslation>['t']) => {
  switch (toolKind) {
    case 'edit':
      return t('tools.kind.edit');
    case 'read':
      return t('tools.kind.read');
    case 'execute':
      return t('tools.shell.displayName');
    default:
      return toolKind;
  }
};
    ...
    <Card className='w-full mb-2' size='small' bordered>
      ...
      <div className='flex items-start gap-3'>
        <div className='flex-1 min-w-0'>
          ...
          <div className='text-xs text-t-secondary mt-2'>{t('tools.labels.toolCallId', { id: tool_call_id })}</div>
        </div>
      </div>
    </Card>
  );
