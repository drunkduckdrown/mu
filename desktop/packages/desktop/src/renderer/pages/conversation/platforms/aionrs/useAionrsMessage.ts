        ...
        case 'tool_group':
          {
            ...
            // If tools are awaiting confirmation, update thought hint. The hint is live and short-lived, so it takes
            // the language of the moment from i18next.
            const confirmingTool = tools.find((tool) => tool.status === 'Confirming');
            if (confirmingTool) {
              setThought({
                subject: i18next.t('conversation.chat.awaitingConfirmation'),
                description: confirmingTool.name || i18next.t('tools.labels.tool'),
              });
            } else if (hasActive) {
              const executingTool = tools.find((tool) => tool.status === 'Executing');
              if (executingTool) {
                setThought({
                  subject: i18next.t('tools.status.executing'),
                  description: executingTool.name || i18next.t('tools.labels.tool'),
                });
              }
