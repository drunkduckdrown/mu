...
const AionrsSendBox: React.FC<{
  ...
  useEffect(() => {
    ...
    void prepareRuntimeSync()
      ...
      .catch((error) => {
        // A translated sentence for an unavailable workspace folder, else "Couldn't start the agent: <backend text>".
  ...
  const executeCommand = useCallback(
    async ({ input, files, sessions }: Pick<ConversationCommandQueueItem, 'input' | 'files' | 'sessions'>) => {
      ...
      try {
        ...
        const errorMessage =
          getConversationRuntimeWorkspaceErrorDetail(error, t) ||
        ...
        // A recognised workspace error is already a translated sentence; anything else is the raw error, shown after
        // a translated headline.
        Message.error(
          normalizeConversationRuntimeWorkspaceErrorCode(error)
            ? errorMessage
            : t('acp.send.failed', { error: errorMessage })
        );
        throw error;
      }
    },
