...
import { getConversationRuntimeWorkspaceErrorDetail } from '@/renderer/pages/conversation/utils/conversationCreateError';
  ...
  const executeCommand = useCallback(
    async ({ input, files, sessions }: Pick<ConversationCommandQueueItem, 'input' | 'files' | 'sessions'>) => {
      ...
      try {
        ...
        const errorMsg =
          getConversationRuntimeWorkspaceErrorDetail(error, t) || parseError(error) || t('common.unknownError');
        ...
        if (isBackendHttpError(error) && error.code === 'CONVERSATION_ARCHIVED') {
          Message.error({
            content: t('conversation.chat.archivedCannotSend'),
            duration: 6000,
          });
...
Please check your local CLI tool authentication status`,
  ...
  // A command the app sends for the person (mu's board switch): now when idle; while busy, the latest one waits
  // here and goes out when the turn ends. Not through the queue: its default mode holds items until sent by hand.
  const waitingCommand = useRef<{ command: string; heard?: (state: SendBoxCommandState) => void }>(undefined);
  const sendCommand = useCallback(
    (command: string, heard?: (state: SendBoxCommandState) => void) => {
      executeCommand({ input: command, files: [] }).then(
        () => heard?.('sent'),
        () => heard?.('dropped')
      );
    },
  ...
  useAddEventListener(
    ...
    (command, target, heard) => {
      if (target !== conversation_id || !command.trim()) return;
      if (!isBusy) {
        sendCommand(command, heard);
        return;
      }
      waitingCommand.current?.heard?.('dropped');
      waitingCommand.current = { command, heard };
      heard?.('waiting');
    },
    [conversation_id, isBusy, sendCommand]
  );
  useEffect(() => {
    const waiting = waitingCommand.current;
    if (isBusy || !waiting) return;
    waitingCommand.current = undefined;
    sendCommand(waiting.command, waiting.heard);
  }, [isBusy, sendCommand]);
  // Leaving the conversation drops a command still waiting: it was never sent.
  useEffect(
    () => () => {
      waitingCommand.current?.heard?.('dropped');
      waitingCommand.current = undefined;
  ...
  const sheetEntries = useMemo<MobileActionSheetEntry[]>(() => {
    ...
    if (runtimeThoughtLevel) {
      entries.push({
        ...
        meta: getCurrentThoughtLevelLabel(t, runtimeThoughtLevel),
        submenu: {
          title: t('agent.thoughtLevel.label'),
          options: runtimeThoughtLevel.options.map((item) => ({
            key: item.value,
            label: thoughtLevelOptionLabel(t, item),
            description: item.description ?? undefined,
      ...
      const mcpOptions: MobileActionSheetOption[] = loadedMcpStatuses.map((item) => ({
        ...
        description:
          item.status === 'loaded'
            ? undefined
            : item.reason
              ? t('conversation.mcp.statusWithReason', {
                  status: t(`conversation.mcp.status.${item.status}` as const),
                  reason: item.reason,
                })
