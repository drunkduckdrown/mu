...
const ChatConversationIndex: React.FC = () => {
  ...
  // Still under the default title ("New Chat", "新会话", …) in any language, so one created before a language switch
  // is retitled too.
  useEffect(() => {
    if (!data || !isDefaultConversationName(data.name)) {
      return;
    }

    void syncTitleFromHistory(data.id);
  }, [data, syncTitleFromHistory]);
