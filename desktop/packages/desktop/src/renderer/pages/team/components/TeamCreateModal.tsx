...
/** A member whose model could not be resolved: its message is already a whole translated sentence naming it. */
class TeamMemberSetupError extends Error {}
...
const TeamCreateModal: React.FC<Props> = ({ visible, onClose, onCreated }) => {
  ...
  const teamCreateHeadline = (detail: string) => t('team.create.errorWithDetail', { error: detail });
  ...
  const handleCreate = async () => {
    ...
    try {
      const resolvedModels = await Promise.all(
        selectedMembers.map(async (member) => {
          try {
            ...
            throw new TeamMemberSetupError(
              t('team.create.memberErrorWithDetail', {
                name: member.assistant.name,
                error: getConversationCreateErrorDetail(error, t),
              }),
              { cause: error }
            );
          }
      ...
      if (result.__bridgeError) {
        Message.error(
          result.message
            ? getConversationCreateErrorMessage(result.message, t, teamCreateHeadline)
            : t('team.create.error')
        );
        return;
      }
      ...
      Message.error(
        error instanceof TeamMemberSetupError
          ? error.message
          : getConversationCreateErrorMessage(error, t, teamCreateHeadline)
      );
    } finally {
      setLoading(false);
    }
