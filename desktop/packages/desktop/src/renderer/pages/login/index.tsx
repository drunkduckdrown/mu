      ...
      if (result.success) {
        ...
        const errorText = (() => {
          switch (result.code) {
            ...
            case 'csrfError':
              return t('login.errors.csrfExpired');
            case 'staleCache':
              return t('login.errors.staleCache');
            case 'unknown':
            default:
              // The server's own message is not in the app language; it stays in the console.
              if (result.message) console.warn('[Login] Sign-in failed:', result.message);
