...
export function formatManagedAgentDiagnosticMessage(t: TFunction, agent: ManagedAgent): string {
  ...
  switch (agent.last_check_error_code) {
    ...
    // arrive in `last_check_error_message` and are put after the explanation
    // rather than into it — the availability snapshot has no structured params
    // column to carry them, and they are the part the user acts on, so they
    // have to survive translation. The brackets around them are the language's.
    ...
    case 'version_drift_newer': {
      ...
      return versions ? t('settings.agentManagement.versionDriftWithVersions', { explanation, versions }) : explanation;
    }
    default:
      return fallback;
  }
