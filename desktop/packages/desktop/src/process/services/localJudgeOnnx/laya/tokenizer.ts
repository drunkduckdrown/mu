// Vendored from mizchi/laya-mlx, web/packages/laya-web/src/tokenizer.ts at dc3aa6b (Apache-2.0).
...
export class LayaTokenizer {
  ...
  constructor(tokenizerJson: TokenizerJson, tokenizerConfig: TokenizerConfig) {
    ...
    if (tokenizerJson.added_tokens.some((a) => a.normalized || a.single_word)) {
      throw new Error('LayaTokenizer requires added tokens with normalized=false and single_word=false');
    }
    ...
    this.addedTokenInfo = new Map(added.map((a) => [a.content, { id: a.id, lstrip: a.lstrip, rstrip: a.rstrip }]));
    ...
    this.pieceSplitter = new RegExp(`${escapeRegExp(this.replacement)}[^${escapeRegExp(this.replacement)}]*`, 'gu');
    const special = (name: keyof TokenizerConfig): [string, number] => {
      const raw = tokenizerConfig[name];
      const content = typeof raw === 'string' ? raw : raw?.content;
      const id =
        content === undefined ? undefined : (this.addedTokenInfo.get(content)?.id ?? this.inner.token_to_id(content));
  ...
  private matchAddedTokens(text: string): { start: number; end: number; id: number }[] {
    ...
    for (const m of text.matchAll(this.contentPattern)) {
      ...
      if (info === undefined) throw new Error('unreachable: contentPattern matched unknown content');
