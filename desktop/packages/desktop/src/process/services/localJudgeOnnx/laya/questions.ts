// Vendored from mizchi/laya-mlx, web/packages/laya-web/src/questions.ts at dc3aa6b (Apache-2.0).
// See ./NOTICE.md. Local changes are marked "mu:".
...
export function toInternal(question: unknown): InternalQuestion {
  ...
  if (typeof kind !== 'string' || !Object.hasOwn(QTYPES, kind)) {
    const repr = kind === undefined ? 'None' : typeof kind === 'string' ? `'${kind}'` : String(kind);
  ...
  if (kind === 'choice') {
    if (Array.isArray(criteria)) {
      ...
      criteria = Object.fromEntries(criteria.map((c): [string, null] => [c, null])); // mu: typed for a build without strictNullChecks
      ...
      if (hasArrayIndexKey && hasOtherKey) {
        throw new Error('Choice criteria with numeric labels must use the list form to preserve option order');
      }
    }
  ...
  // mu: the cast states what the checks above proved; without strictNullChecks TypeScript cannot narrow it.
  return { t: 'noul', ins: computeIns(), crit: (criteria ?? null) as { false?: Criterion; true?: Criterion } | null };
}
...
const isBlank = (v: Criterion | undefined): v is null | undefined | '' => v === null || v === undefined || v === '';
...
export function renderOptions(q: InternalQuestion): string[] {
  ...
  return [
    'false: ' + (isBlank(falseCrit) ? 'no, the statement does not hold' : renderCriterion(falseCrit)),
