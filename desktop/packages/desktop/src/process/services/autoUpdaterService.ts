...
import { classifyUpdateError, type UpdateErrorInfo } from '@/common/update/updateErrors';
...
/** Result of a service step; on failure `errorInfo` says why and `error` carries the raw message. */
type ServiceResult = { success: boolean; error?: string; errorInfo?: UpdateErrorInfo };
...
class AutoUpdaterService extends EventEmitter {
  ...
  private _activeDownloadPromise: Promise<ServiceResult> | null = null;
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        'provider: generic',
        `url: ${cdnFeedOptions.url}`,
        'updaterCacheDirName: io.github.qybaihe.mu',
        '',
      ].join('\n');
   ...
   * while the downloaded archive contains the packaged app (io.github.qybaihe.mu). Squirrel.Mac
  ...
  private async handleNativeInstallError(error: Error): Promise<void> {
    ...
    this.broadcastStatus({
      status: 'error',
      error: userMessage,
      errorInfo: { code: 'prepareInstallFailed' },
    });
    const wait = this._nativeInstallReadyWait;
    this.clearNativeInstallReadyWait();
  ...
  private async waitForNativeInstallReady(): Promise<void> {
    ...
    const timer = setTimeout(async () => {
      ...
      this.broadcastStatus({
        status: 'error',
        error: userMessage,
        errorInfo: { code: 'prepareInstallTimeout' },
      });
      const wait = this._nativeInstallReadyWait;
      this.clearNativeInstallReadyWait();
      wait?.reject(new Error(userMessage));
  ...
  async checkForUpdates(): Promise<ServiceResult & { updateInfo?: UpdateInfo }> {
    try {
      ...
      if (!result) {
        ...
        return {
          success: false,
          error: i18n.t('update.errors.checkReturnedNull'),
          errorInfo: { code: 'checkUnavailable' },
        };
      }
  ...
  async restoreDownloadedUpdateIfAvailable(): Promise<ServiceResult & { data: AutoUpdateReadyResult }> {
    try {
      if (!this._isInitialized) {
        throw new Error('AutoUpdaterService not initialized');
      }
      ...
      if (!downloadResult.success) {
        return {
          ...
          errorInfo: downloadResult.errorInfo,
        };
      }
  ...
  async downloadUpdate(): Promise<ServiceResult> {
    ...
    const runDownload = async (): Promise<ServiceResult> => {
      try {
        if (!this._isInitialized) {
          throw new Error('AutoUpdaterService not initialized');
        }
  ...
  async cancelDownload(): Promise<ServiceResult> {
    if (!this._activeDownloadPromise) {
      this.broadcastStatus({ status: 'cancelled' });
      return { success: true };
    }
