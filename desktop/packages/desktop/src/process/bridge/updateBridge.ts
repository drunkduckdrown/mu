...
/**
 * Failures carry an UpdateErrorInfo code, which the renderer shows as translated text in the current app language.
 * The messages thrown here are plain English details for the log.
 */
const failure = (error: unknown): { success: false; msg: string; errorInfo: UpdateErrorInfo } => ({
  success: false,
  msg: updateErrorDetail(error),
  errorInfo: classifyUpdateError(error),
});
...
export const latestRelease = (releases: GitHubReleaseApi[]): UpdateReleaseInfo | null => {
  ...
  const assets: GitHubReleaseAsset[] = (release.assets ?? [])
    .filter((asset) => asset && isAllowedAssetName(asset.name))
    .map((asset) => ({
      name: asset.name,
...
const assertAllowedUrl = async (rawUrl: string) => {
  ...
  try {
    ...
    throw new UpdateError({ code: 'invalidUrl' }, `Invalid download URL: ${rawUrl}`);
  }

  if (parsed.protocol !== 'https:') {
    throw new UpdateError({ code: 'httpsOnly' }, `Only HTTPS download URLs are allowed: ${rawUrl}`);
  }
  if (!ALLOWED_DOWNLOAD_HOSTS.has(parsed.hostname)) {
    throw new UpdateError(
      { code: 'hostNotAllowed', host: parsed.hostname },
...
const fetchWithAllowlistedRedirects = async (rawUrl: string, signal: AbortSignal): Promise<Response> => {
  ...
  for (let i = 0; i <= MAX_REDIRECTS; i++) {
    ...
    if (res.status >= 300 && res.status < 400) {
      ...
      if (!location) {
        throw new UpdateError({ code: 'redirectNoLocation' }, 'Download redirect did not include a Location header');
      }
  ...
  throw new UpdateError({ code: 'tooManyRedirects' }, 'Too many download redirects');
};
...
const fetchGitHubReleases = async (repo: string, timeoutMs = 30000): Promise<GitHubReleaseApi[]> => {
  ...
  try {
    ...
    if (!res.ok) {
      throw new UpdateError(
        res.status === 404 ? { code: 'noUpdateInfo' } : { code: 'serverError', status: res.status },
    ...
    if (!Array.isArray(json)) {
      throw new UpdateError({ code: 'invalidMetadata' }, `GitHub API response is not a release list: ${url}`);
    }
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `mu-update-${Date.now()}`;
};
...
const attemptDownload = async (
  ...
  try {
    ...
    if (!res.body) {
      throw new UpdateError({ code: 'downloadNoBody' }, 'Download response did not include a body');
    }
    ...
    return { ok: false, isAbort, message, errorInfo: classifyUpdateError(err), receivedBytes, totalBytes };
  }
};
  ...
  try {
    ...
    if (finalResult.ok) {
      ...
      emitProgress({
        ...
        errorInfo: finalResult.errorInfo,
      });
    }
  } finally {
...
export function initUpdateBridge(): void {
  ...
  ipcBridge.update.check.provider(
    async (
      params
    ): Promise<{ success: boolean; data?: UpdateCheckResult; msg?: string; errorInfo?: UpdateErrorInfo }> => {
      try {
        const repo = resolveRepo(params?.repo);
        const currentVersion = app.getVersion();

        // Update comparisons are pure semver: `app.getVersion()` (the packaged app's version) against the newest
        // release tag. A release is offered only when its tag is a higher version than the build that is running.
        log.info('[manual-update] Checking releases of', repo);
        const latest = latestRelease(await fetchGitHubReleases(repo));
        ...
        return {
          success: true,
          data: { currentVersion, updateAvailable: semver.gt(latest.version, currentSemver), latest },
        };
      } catch (err: unknown) {
        log.warn('[manual-update] Update check failed:', updateErrorDetail(err));
        return failure(err);
      }
    }
  );
  ...
  ipcBridge.update.download.provider(
    ...
    ): Promise<{ success: boolean; data?: UpdateDownloadResult; msg?: string; errorInfo?: UpdateErrorInfo }> => {
      try {
        if (!params?.url) {
          return failure(new UpdateError({ code: 'missingUrl' }, 'Missing download URL'));
        }
        ...
        log.warn('[update-download] Download refused:', updateErrorDetail(err));
        return failure(err);
      }
    }
  );
  ...
  ipcBridge.update.cancelDownload.provider(
    async (
      params: UpdateDownloadCancelRequest
    ): Promise<{ success: boolean; msg?: string; errorInfo?: UpdateErrorInfo }> => {
      try {
        const downloadId = params?.downloadId;
        if (!downloadId) {
          return failure(new UpdateError({ code: 'missingDownloadId' }, 'Missing download ID'));
        }
  ...
  ipcBridge.autoUpdate.check.provider(
    async (
      ...
      try {
        ...
        return { success: result.success, msg: result.error, errorInfo: result.errorInfo };
      } catch (err: unknown) {
        return failure(err);
      }
    }
  ...
  ipcBridge.autoUpdate.download.provider(
    async (): Promise<{ success: boolean; msg?: string; errorInfo?: UpdateErrorInfo }> => {
      try {
  ...
  ipcBridge.autoUpdate.restoreDownloaded.provider(
    async (): Promise<{ success: boolean; data: AutoUpdateReadyResult; msg?: string; errorInfo?: UpdateErrorInfo }> => {
      try {
        const result = await autoUpdaterService.restoreDownloadedUpdateIfAvailable();
        return { success: result.success, data: result.data, msg: result.error, errorInfo: result.errorInfo };
      } catch (err: unknown) {
        return { ...failure(err), data: { ready: false } };
      }
    }
  );

  ipcBridge.autoUpdate.cancelDownload.provider(
    async (): Promise<{ success: boolean; msg?: string; errorInfo?: UpdateErrorInfo }> => {
      try {
