...
export const contentText = (content: unknown): string => typeof content === 'string' ? content : list(content).map(block => record(block).type === 'text' ? str(record(block).text) : '').filter(Boolean).join('\n');
export interface Envelope { sessionId: string; sequence: number; at: number; event: RecordValue }
export interface SessionMeta { id: string; title: string; cwd: string }
export interface Model { id: string; provider: string; name: string }
export interface SessionSnapshot extends SessionMeta { events: Envelope[]; busy: boolean; state?: RecordValue; models: Model[]; dialogs: RecordValue[] }
export interface TimelineItem { id: string; type: 'user' | 'assistant' | 'tool' | 'verdict' | 'notice'; text: string; data?: RecordValue; running?: boolean }
export interface ViewState { items: TimelineItem[]; phase: string; judge: string; decisions: RecordValue[]; swarm?: RecordValue; busy: boolean; dialogs: RecordValue[] }
...
export function foldEvents(events: Envelope[]): ViewState {
  ...
  for (const envelope of events) {
    ...
    if (e.type === 'kyrn_submission') {
      ...
      phase = e.behavior ? '消息已进入队列' : '已接收，准备分类'; busy = true;
    } else if (e.type === 'agent_start') { phase = '正在执行'; busy = true; }
    else if (e.type === 'kyrn_host_ready') { phase = '就绪'; busy = false; dialogs.clear(); }
    else if (e.type === 'kyrn_state') { busy = e.busy === true; if (!busy) phase = '就绪'; }
    else if (e.type === 'agent_settled') { phase = '本轮完成'; busy = false; }
    else if (e.type === 'message_start' && record(e.message).role === 'assistant') {
      streaming = { id, type: 'assistant', text: '', running: true }; items.push(streaming);
    } else if (e.type === 'message_update') {
      const delta = record(e.assistantMessageEvent);
      if (delta.type === 'text_delta') {
        if (!streaming) { streaming = { id, type: 'assistant', text: '', running: true }; items.push(streaming); }
      ...
      if (streaming) { streaming.text = text; streaming.running = false; }
      ...
      const item: TimelineItem = { id, type: 'tool', text: str(e.toolName), data: { args: e.args, toolCallId: e.toolCallId }, running: true };
      tools.set(str(e.toolCallId), item); items.push(item); phase = `执行 ${str(e.toolName)}`;
      ...
      if (item) { item.data = { ...item.data, result: contentText(result.content), isError: e.isError }; item.running = e.type !== 'tool_execution_end'; }
      ...
      if (e.method === 'setStatus' && e.statusKey === 'kyrn.presentation.v1') {
        ...
        if (p.kind === 'preflight.pending') {
          ...
          const item = verdicts.get(key);
          if (item?.running) { item.running = false; item.data = { state: 'none', reason: payload.reason, latencyMs: payload.waitedMs }; }
      ...
      items.push({ id, type: 'notice', text: str(e.message || e.reason) }); busy = false; phase = '需要关注';
      if (e.type === 'kyrn_host_exit') dialogs.clear();
    } else if (e.type === 'auto_retry_start') phase = `请求重试 ${num(e.attempt)}/${num(e.maxAttempts)}`;
  }
  return { items: items.filter(item => item.text || item.type !== 'assistant'), phase, judge, decisions, swarm, busy, dialogs: [...dialogs.values()] };
}

declare global {
  interface Window {
    kyrn: { request<T = unknown>(method: string, args?: RecordValue): Promise<T>; subscribe(listener: (event: Envelope) => void): () => void };
  }
}
