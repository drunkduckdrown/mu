...
export class Preferences {
  constructor(home, agentDir) { this.path = join(home, 'preferences.json'); this.agentDir = agentDir; }
  read() {
  ...
  effective(cwd) { const config = this.read(); return { ...config.defaults, ...object(config.projects[cwd]) }; }
  save(scope, cwd, values) {
    ...
    for (const [key, value] of Object.entries(object(values))) {
      if (!allowed.includes(key) || typeof value !== 'string' || value.length > 300 || /[\r\n\0]/.test(value)) throw new Error('无效配置字段');
      ...
      if (key === 'judge' && value.split(',').some(name => !available.includes(name))) throw new Error('未知判断模型');
