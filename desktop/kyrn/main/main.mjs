...
let catalog = [];
try { catalog = JSON.parse(readFileSync(indexPath, 'utf8')); } catch {}
...
function emit(session, event) {
  ...
  if (event.type === 'extension_ui_request' && ['confirm', 'select', 'input', 'editor'].includes(event.method)) session.dialogs.set(event.id, event);
...
async function attach(meta) {
  ...
  if (existsSync(journal(meta.id))) {
    try { events = readFileSync(journal(meta.id), 'utf8').trim().split('\n').filter(Boolean).map(line => JSON.parse(line)).slice(-6000); } catch {}
  }
  ...
  session.host = new RpcHost({ launcher, cwd: meta.cwd, session: meta.file,
    env: { KYRN_JUDGE: config.judge, KYRN_JUDGE_MODE: config.mode }, onEvent: event => emit(session, event) });
  try {
    ...
    if (!meta.file) {
      ...
      if (available.levels.includes(config.thinking)) await session.host.send({ type: 'set_thinking_level', level: config.thinking });
    }
  ...
  } catch (error) { emit(session, { type: 'kyrn_error', message: error.message }); }
  return session;
}
...
const snapshot = session => ({ id: session.id, cwd: session.cwd, title: session.title, events: session.events, busy: session.busy, state: session.state, models: session.models || [], levels: session.levels || [], dialogs: [...session.dialogs.values()] });

const handlers = {
  bootstrap: async () => ({ root, ready: existsSync(launcher), sessions: catalog.map(meta => ({ id: meta.id, cwd: meta.cwd, title: meta.title })) }),
  create: async ({ cwd: existing }) => {
    let selected = existing;
    if (selected && !catalog.some(item => item.cwd === selected)) throw new Error('请通过目录选择器添加项目');
    if (!selected) {
      const picked = await dialog.showOpenDialog(window, { title: '选择 KYRN 项目', defaultPath: root, properties: ['openDirectory'] });
      if (picked.canceled) return null;
      selected = picked.filePaths[0];
    }
    const cwd = await realpath(selected);
    const meta = { id: randomUUID(), cwd, title: '新任务' };
    catalog.unshift(meta); saveIndex();
  ...
  prompt: async ({ id, text, behavior }) => {
    ...
    session.busy = true;
    const meta = catalog.find(item => item.id === id);
    if (meta.title === '新任务') { meta.title = text.slice(0, 36); session.title = meta.title; saveIndex(); }
    ...
    try {
      ...
      void session.host.send({ type: 'prompt', message: text, ...(midRun ? { streamingBehavior: behavior === 'steer' ? 'steer' : 'followUp' } : {}) })
        .then(async () => {
        ...
        .catch(error => { session.busy = false; emit(session, { type: 'kyrn_error', message: error.message }); })
        .finally(() => { session.submitting = false; });
      return { accepted: true };
    } catch (error) { session.busy = midRun; session.submitting = false; throw error; }
  },
  ...
  model: async ({ id, provider, modelId }) => {
    ...
    if (!session.models.some(model => model.provider === provider && model.id === modelId)) throw new Error('模型不在已登录的可用列表中');
  ...
  files: async ({ id }) => {
    ...
    const result = await run('git', ['ls-files', '-co', '--exclude-standard'], { cwd, timeout: 10000, maxBuffer: 2 * 1024 * 1024 });
    return [...new Set(result.stdout.split('\n'))].filter(path => path && !/(^|\/)(\.env($|\.)|auth\.json$|node_modules\/)/.test(path)).slice(0, 3000);
  },
  ...
  diff: async ({ id }) => {
    ...
    const paths = ['--', '.', ':(exclude)**/.env', ':(exclude)**/.env.*', ':(exclude)**/auth.json', ':(exclude)**/*.pem', ':(exclude)**/*.key'];
    const [working, staged] = await Promise.all([run('git', [...args, ...paths], opts), run('git', [...args, '--cached', ...paths], opts)]);
    return { working: working.stdout, staged: staged.stdout };
  },
};

function createWindow() {
  window = new BrowserWindow({ width: 1460, height: 960, minWidth: 1000, minHeight: 680, title: 'KYRN', backgroundColor: '#f8f9fb', titleBarStyle: 'hiddenInset', webPreferences: { preload: join(here, 'preload.cjs'), contextIsolation: true, nodeIntegration: false, sandbox: true } });
...
ipcMain.handle('kyrn:request', async (event, method, args = {}) => {
  if (!window || event.sender !== window.webContents || event.senderFrame !== window.webContents.mainFrame) throw new Error('Invalid caller');
  if (!Object.hasOwn(handlers, method)) throw new Error('Unknown command');
  return handlers[method](args);
});
createWindow();
});
app.on('second-instance', () => { if (!window || window.isDestroyed()) createWindow(); else { window.show(); window.focus(); } });
app.on('activate', () => { if (BrowserWindow.getAllWindows().length === 0) createWindow(); });
let quitting = false;
app.on('before-quit', event => {
  if (!quitting && [...sessions.values()].some(session => session.busy)) {
    event.preventDefault();
    void dialog.showMessageBox({ type: 'question', message: '有任务正在运行，退出将停止这些任务。', buttons: ['继续运行', '停止并退出'], defaultId: 0, cancelId: 0 }).then(result => {
