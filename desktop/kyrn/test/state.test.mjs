...
const presentation = (kind, payload, sequence) => envelope({ type: 'extension_ui_request', method: 'setStatus', statusKey: 'kyrn.presentation.v1', statusText: JSON.stringify({ runtimeId: 'r', turnId: 1, kind, payload }) }, sequence);

test('pending and verdict occupy one native card, with no invented reasoning', () => {
  const events = [envelope({ type: 'kyrn_submission', text: 'hi' }), presentation('preflight.pending', { judge: 'jev' }, 2)];
  assert.equal(foldEvents(events).items[1].running, true);
  events.push(presentation('preflight.verdict', { state: 'applied', turnType: 'chat', gear: 'chat', latencyMs: 42 }, 3));
...
test('tool snapshots expose bee progress and final authoritative messages replace deltas', () => {
  const state = foldEvents([
    ...
    envelope({ type: 'tool_execution_update', partialResult: { details: { snapshot: { bees: [{ name: 'a', status: 'tool' }] } } } }, 4),
...
test('timeout is shown as fallback, late verdict does not claim to have been applied', () => {
  const events = [presentation('preflight.pending', { judge: 'jev' }, 1), presentation('preflight.wait_end', { reason: 'timeout', waitedMs: 6000 }, 2)];
