...
test('RPC matches replies by id and streams events independently', async () => {
  ...
  const host = new RpcHost({ launcher: 'fixture', cwd: '/tmp', onEvent: event => events.push(event), spawnProcess: () => child });
  const first = host.send({ type: 'get_state' });
  const second = host.send({ type: 'get_available_models' });
  child.stdout.write(JSON.stringify({ type: 'response', id: commands[1].id, success: true, data: { models: [] } }) + '\n');
  child.stdout.write('not json\n' + JSON.stringify({ type: 'agent_start' }) + '\n');
  child.stdout.write(JSON.stringify({ type: 'response', id: commands[0].id, success: true, data: { sessionFile: 'fixture' } }) + '\n');
  ...
  child.stdout.end(); child.stdin.end(); child.stderr.end();
});
