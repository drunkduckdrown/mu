...
KYRN's harness layer for [pi](https://github.com/earendil-works/pi): a small, fast judgment model makes the routine calls (what kind of turn is this, which tool output matters, which sub-agent and model fit a task, what to click next on a web page), code carries them out, and the main model keeps its context for thinking.
...
From this repository, `kyrn/bin/kyrn-dev` runs pi from source with the full extension and its own agent directory (`~/.kyrn/agent`).

## Configure it

`<agent dir>/kyrn.json`, all optional:
...
Environment: `KYRN_JUDGE=laya,llm:provider/model` (or `off`), `KYRN_JUDGE_MODE=active`. In a session: `/kyrn`, `/kyrn judge laya,jev`, `/kyrn mode tool.admission active`, `/agents`, `/remember`.

## What the main model gets

| Tool | What happens behind it |
| --- | --- |
| `browse({ url, goal })` | A real Chrome in KYRN's own profile. The judge picks every operation and target from the elements on the page until the goal is visibly met; only the final page and the action trace come back. Needs a judge that can relate a goal to a page (Jev or an `llm:` tier). |
...
Own Chrome profile (`~/.kyrn/browser-profile`), never the user's. Password, file and hidden inputs are never offered to the judge. Clicks that look irreversible (pay, delete, send) need confirmation and stop when nobody can give it. Page text is passed along as untrusted data. Closed shadow roots, iframes and canvas-only interfaces are not observed.
