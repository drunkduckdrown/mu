...
export const MANIFEST: HarnessManifest = {
	...
	decisions: [
		...
		},
		{
			...
			summary: {
				zh: "打开人话看板的项目里，代理每做几步、以及每次停下时，用选择题读出它处在哪个阶段、在做哪条验收条件、是不是在等你、和上次比有没有新情况。只有出现新情况，才请会说人话的模型重写看板。",
				en: "In a project with the board on, every few tool calls and whenever the agent stops, multiple-choice questions read its phase, the acceptance item it works on, whether it waits for you, and whether anything changed since last time. Only something new has a plain-speaking model write the board again.",
			},
		},
		{
			id: "notify.routing",
	...
	features: [
		...
		},
		{
			...
			summary: {
				zh: "用大白话告诉你项目推进到哪、现在在干什么、有什么要你确认。Jev 用选择题读出要点，会说人话的模型来写；只给人看，不进模型的上下文。每个项目单独开关（/board on、/board off）。",
				en: "Tells you in plain words how far the work is, what is happening now and what waits on you. Jev reads the facts by multiple choice, a plain-speaking model writes; for you only, never in the model's context. Switched per project (/board on, /board off).",
			},
			defaultEnabled: true,
			options: [
				{
				...
				},
				{
					...
					help: {
						zh: "写成“提供商/模型”，选一个表达好的，比如 Gemini Flash。留空用 writer 模型，再没有就用当前会话的模型。",
						en: "As provider/model; pick one that explains well. Empty: the writer model, else the session's model.",
					},
				},
				{
					key: "language",
