...
コードを読まない人でも、エージェントがどこまで進んだかがわかります。ボードがオンのとき（`/board`、またはデスクトップアプリのボードタブのスイッチ）、`board.read` は数回のツール呼び出しごと、検証のあとや受け入れ条件の項目にチェックが入ったあと、そしてエージェントが止まるたびに実行されます。そのあと説明の上手なモデルが、三つのことをわかりやすい言葉で伝えます。いま何をしているか、チェックリストの項目のうちいくつが終わったか、あなたの確認を待っているものは何か。それまでの更新は下に順番に残り、実行が終わるとまとめが付きます。
...
この文章は、モデルの発言を縮めたものではありません。判定器が判定とイベントの中から本当にニュースと呼べるものをいくつか選び、ボードは権限モード、目標、サブエージェントに合わせて変わります。上部の二つの数字はコンテキストの使用量とキャッシュヒット率で、上に挙げたコンテキストとキャッシュの判定の直接の結果です。
