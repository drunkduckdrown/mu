...
フロンティアモデルは、作業の腕は上がるのに、それを語るのは下手になっていきます。世代が進むたびに、進捗報告は別の機械に向けた出力のようになり、より密に、より不透明になって、人が話す言葉から遠ざかっていきます。mu は、作業をするモデル自身には語らせません。ボードがオンのとき（`/board`、またはデスクトップアプリのボードタブのスイッチ）、`board.read` は数回のツール呼び出しごと、検証のあとや受け入れ条件の項目にチェックが入ったあと、そしてエージェントが止まるたびに実行されます。そのたびに判定器が、判定とイベントの中から本当にニュースと呼べるものをいくつか選び出します。次に、わかりやすく話すという一点だけで選ばれたモデル（`/board model`）が、三つのことを伝えます。いま何をしているか、チェックリストの項目のうちいくつが終わったか、あなたの確認を待っているものは何か。それまでの更新は下に順番に残り、実行が終わるとまとめが付きます。
