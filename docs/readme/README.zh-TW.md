...
不看程式碼的人也能知道代理做到哪了。開啟看板（`/board`，或桌面版看板頁上的開關）後，`board.read` 會在每隔幾次工具呼叫、每次檢查或勾掉一條驗收項目之後、以及代理每次停下時執行一次。然後由一個說話清楚的模型用白話說三件事：現在在做什麼，清單上幾件事做完了幾件，有什麼在等你。之前的每一段按順序留在下面；一次執行結束時會有一段總結。

<p align="center"><img src="board.png" width="960" alt="桌面版的人話看板：進展到哪、在做什麼、之前發生了什麼；頂端是上下文用量和快取命中率"></p>

這些字不是模型原話的縮寫：判定器從判定和事件裡挑出真正算得上新消息的那幾條，看板跟著權限模式、目標和子代理走。頂端兩個數字是上下文用量和快取命中率，正是上面那些上下文和快取判定的直接結果。
