...
프런티어 모델은 일은 더 잘하게 되는데, 그 일에 대해 말하는 건 더 서툴러져요. 세대가 바뀔수록 진행 보고는 다른 기계가 읽을 출력에 가까워지고, 더 빽빽하고 더 불투명해져서 사람이 하는 말과는 멀어져요. mu는 일하는 모델에게 스스로 보고하게 하지 않아요. 보드를 켜 두면(`/board`, 또는 데스크톱 앱 보드 탭의 스위치), `board.read`가 도구 호출 몇 번마다, 검증을 하거나 인수 조건 항목에 체크한 뒤, 그리고 에이전트가 멈출 때마다 실행돼요. 그때마다 판정기가 판정과 이벤트 가운데 정말 새 소식이라 할 만한 몇 가지를 골라요. 그다음, 쉽게 말한다는 한 가지 이유만으로 고른 모델(`/board model`)이 세 가지를 알려 줘요. 지금 무엇을 하고 있는지, 체크리스트 항목 중 몇 개가 끝났는지, 여러분을 기다리는 것이 무엇인지요. 이전 업데이트는 아래에 순서대로 남고, 실행이 끝나면 요약이 붙어요.
