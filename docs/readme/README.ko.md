...
코드를 읽지 않는 사람도 에이전트가 어디까지 했는지 알 수 있어요. 보드를 켜 두면(`/board`, 또는 데스크톱 앱 보드 탭의 스위치), `board.read`가 도구 호출 몇 번마다, 검증을 하거나 인수 조건 항목에 체크한 뒤, 그리고 에이전트가 멈출 때마다 실행돼요. 그러면 설명을 잘하는 모델이 쉬운 말로 세 가지를 알려 줘요. 지금 무엇을 하고 있는지, 체크리스트 항목 중 몇 개가 끝났는지, 여러분을 기다리는 것이 무엇인지요. 이전 업데이트는 아래에 순서대로 남고, 실행이 끝나면 요약이 붙어요.
...
이 글은 모델이 한 말을 줄인 것이 아니에요. 판정기가 판정과 이벤트 가운데 정말 새 소식이라 할 만한 몇 가지를 고르고, 보드는 권한 모드, 목표, 서브 에이전트를 따라가요. 맨 위의 두 숫자는 컨텍스트 사용량과 캐시 적중률로, 위에서 본 컨텍스트와 캐시 판정의 직접적인 결과예요.
