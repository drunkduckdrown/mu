...
不看代码的人也能知道代理做到哪了。打开看板（`/board`，或桌面端看板页上的开关）后，`board.read` 每隔几次工具调用、每次检查或勾掉一条验收项之后、以及代理每次停下时跑一次。然后由一个说话清楚的模型用大白话说三件事：现在在做什么，清单上几件事做完了几件，有什么在等你。之前的每一段按顺序留在下面；一次运行结束时有一段总结。

<p align="center"><img src="board.png" width="960" alt="桌面端的人话看板：进展到哪、在做什么、之前发生了什么；顶上是上下文占用和缓存命中率"></p>

这些字不是模型原话的缩写：判定器从判定和事件里挑出真正算新消息的那几条，看板跟着权限模式、目标和子代理走。顶上两个数是上下文占用和缓存命中率，正是上面那些上下文和缓存判定的直接结果。
