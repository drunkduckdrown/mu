...
大模型一代代迭代下来，干活越来越强，说话却越来越黑：汇报进展的话越写越像给机器看的，术语堆叠，越来越不说人话。mu 不让干活的模型自己汇报。打开看板（`/board`，或桌面端看板页上的开关）后，`board.read` 每隔几次工具调用、每次检查或勾掉一条验收项之后、以及代理每次停下时跑一次，判定器从判定和事件里挑出真正算新消息的那几条。再由一个只因为说人话而选出来的模型（`/board model` 可换）讲三件事：现在在做什么，清单上几件事做完了几件，有什么在等你。之前的每一段按顺序留在下面；一次运行结束时有一段总结。
